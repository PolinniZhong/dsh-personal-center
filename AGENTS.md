...
- **v0.8 候选**(详见 docs/个性化指令增强-v0.8-PRD.md,含细化候选清单 + 对抗式审查):
  - 条件注入:A1 按模型(P0,信号已确认)、A2 按时段(P0)、A3 按任务/A4 按文件(信号待内核,backlog);
  - 模板导入导出:B1 导出 JSON、B2 导入(校验+去重+覆盖策略)、B3 分享(可选);
  - 一期建议范围:A1+A2+B1+B2;A3/A4/B3 后置。
