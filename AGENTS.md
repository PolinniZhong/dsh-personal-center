...
1. **下拉交互必须"点击即弹出列表"**:禁止用 `datalist`(DSH 设置面板 iframe 中不触发下拉,
   用户无法使用,已返工一次)。用自定义弹出层(picker + caret + menu)。
...
4. **注入预览**:**默认显示全局指令**,工作区指令折叠在「⋯ 查看工作区指令」下;
   位于模板库上方;紧凑(限高 + 滚动);无底部说明文字。
5. **模板库每个模板独立描边框**(一格一框,像绘画库),不用一条线分隔。
6. **工作区路径短显示**「…/名称」,点击展开完整路径。
7. **避免重复控件**(同一区域出现两个相同功能的输入框 = bug,通常是 key 冲突/发现逻辑问题)。
8. **修改后自查交互是否真实可用**:不只跑语法/冒烟,还要代入用户视角检查"点击是否真的能弹出、
