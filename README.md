...
一个面向 DeepSeek Harness(DSH)桌面端 / Web 端的本地插件。在设置里新增「**个人**」分区,含四个外层 tab:
...
- **宠物**:浮游小黑鲸 —— 由真实用量数据驱动 5 种情绪(开心/忙碌/疲惫/钱包痛/打盹),点击吐数据气泡、可拖拽并记忆位置、右键快捷菜单,30 秒轮询。
...
- 界面右下角浮游小黑鲸,纯前端零依赖(素材为宿主托管的动画 WebP,真 alpha 透明,深色主题有描边光);
- **默认静止不打扰**(参考 Codex 宠物设计):待机时显示单帧静态表情,不做循环动作;**鼠标悬停或点击才按当前情绪播放一次小动作**,约 2 秒后回到静止;
...
- **自由拖拽**:拖到哪就停在哪,松手后位置记忆到宿主配置(刷新后恢复),窗口缩放自动钳制在视口内;面板可一键「重置到右下角」;
- **右键**快捷菜单:改尺寸(小/中/大)、隐藏宠物;
- 「宠物」tab 可配置:启用开关、尺寸、位置(四角 + 自定义拖拽位置)、**透明度(可随时来回调节)**;
...
| v0.4 | 桌面宠物(小黑鲸,真实数据驱动 5 情绪 + 配置面板) | ✅ 可用 |
| v0.5 | 宠物更多皮肤/表情、用量导出(JSON/CSV)、月度/年度对比 | 💡 构想 |
...
│   ├── client.js         # 浏览器:「个人」分区 UI(统计 + 个性化 + 宠物面板/浮层)
