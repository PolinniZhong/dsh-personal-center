...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		class PetWidget {
			...
			_bindEvents() {
				this._onClick = (e) => {
					...
					this.playAction();
					// 状态概览:点宠物 toggle(开/关);拖拽中不触发(上面 moved 已拦截)。
					// 打开面板时不弹随机 stats 气泡(面板内状态气泡/摘要已表达);关闭时保留随机气泡。
		...
		class PetStatusPanel {
			...
			_project() {
				...
				for (const id of snap.ids || []) {
					...
					if (!entry) continue;
					const s = this.sessions.get(id);
					const failed = !!(s && s.snapshot().lastAgentError);
					let status = "idle";
		...
		function apply(ctx) {
			...
			// 可选服务:未挂载时面板不启用(点击宠物仅保留原气泡行为),不阻塞宠物。
