 ...
 *   顶栏标题「个人配置」→ 描述 → Tab 栏(Token 用量 / 个性化 / 宠物)切换内容:
 *   - 「Token 用量」:Token 消耗 / 会话 / 工具调用 / Token 活动(每日·每周·累计)
 *     / 按模型分布 / 常用工具 / 成本 —— 数据来自宿主端环回路由 /personal-center/stats;
 *   - 「个性化」:自定义指令多行文本框(全局注入,见宿主端);
 *   - 「宠物」:单页自上而下 = 会话状态设置卡 + 黑鲸/蓝鲸两张配置卡 + 全局浮层
 *     (纯 DOM 零依赖,素材为宿主托管的动画 WebP,30s 轮询 stats 驱动 5 情绪;
 *     会话状态开关控制状态概览按钮 + 常驻情绪同步)。
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			/* 会话状态设置卡(带边框:标题+开关一行,说明在下,参考设置页 toggle 卡样式) */
			".dsh-pc-ss-card{border:1px solid var(--dsw-alias-border-l2);border-radius:12px;padding:14px 16px;margin-bottom:16px}" +
			".dsh-pc-ss-row{display:flex;align-items:center;justify-content:space-between;gap:12px}" +
			".dsh-pc-ss-title{margin:0;color:var(--dsw-alias-label-primary);font-size:14px;font-weight:500;line-height:22px}" +
			".dsh-pc-ss-desc{margin:6px 0 0;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}" +
			...
			".dsh-pet-status-item{display:flex;align-items:center;gap:8px;padding:7px 8px;border-radius:8px;font-size:12px;line-height:18px;cursor:pointer}" +
		...
		class PetStatusPanel {
			...
			_render() {
				...
				// 列表只展示前 5 条(rows.slice(0,5)),故先按状态优先级排序:运行中/失败置顶,
				// 否则正运行中的会话可能排在 idx>=5 而被截断——摘要计数按全量统计、列表却看不到
				// (进行中 1 但 5 条全待机)。排序只影响展示顺序,不影响上方摘要计数。
		...
		function PetStatusConfig({ t, petCtl }) {
			...
			return react.createElement("div", { className: "dsh-pc-ss-card" },
				react.createElement("div", { className: "dsh-pc-ss-row" },
					react.createElement("span", { className: "dsh-pc-ss-title" }, t("petSubStatus")),
				...
				react.createElement("p", { className: "dsh-pc-ss-desc" }, t("petStatusDesc")),
		...
		/**
		 * 「个人配置」分区:标题 + 描述 + Tab 栏(Token 用量 / 个性化 / 宠物)。
		 * @param {{ t: (key: string) => string }} props
		 */
