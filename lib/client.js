...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PetPanel({ t, petCtl }) {
			...
			// 宠物列表:黑鲸/蓝鲸各一张完整配置卡,点击卡片切换当前宠物(与小黑鲸原卡片同级)
			const petCard = (id) => {
				const isActive = curSkin === id;
				const asset = emotion
					? PET_ASSET_BASE + id + "/idle/" + emotion + ".webp"
					: PET_ASSET_BASE + id + "/idle/happy.webp";
				return react.createElement("div", {
					className: "dsh-pc-pet-card dsh-pc-pet-pick" + (isActive ? " active" : ""),
					"data-active": isActive,
					onClick: () => { if (!isActive) update({ skin: id }); }
				},
					...
					react.createElement("div", { className: "dsh-pc-pet-emo-bubble" },
						emotion ? react.createElement("img", { className: "dsh-pc-pet-emo-img", src: PET_ASSET_BASE + id + "/idle/" + emotion + ".webp", alt: "" }) : null,
