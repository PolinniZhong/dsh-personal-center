...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-pet-bubble{position:absolute;bottom:calc(100% + 12px);left:50%;transform:translateX(-50%) translateY(8px);background:rgba(255,255,255,.6);-webkit-backdrop-filter:blur(12px) saturate(1.4);backdrop-filter:blur(12px) saturate(1.4);color:var(--dsw-alias-label-primary);border:1px solid rgba(255,255,255,.5);padding:10px 16px;border-radius:16px;font-size:13px;white-space:nowrap;box-shadow:0 4px 20px rgba(0,0,0,.18);opacity:0;pointer-events:none;transition:opacity .25s ease,transform .25s ease;max-width:min(280px,60vw);overflow:hidden;text-overflow:ellipsis}" +
			".dsh-pet-bubble.visible{opacity:1;transform:translateX(-50%) translateY(0)}" +
			".dsh-pet-bubble::after{content:\"\";position:absolute;top:100%;left:50%;transform:translateX(-50%);border:6px solid transparent;border-top-color:rgba(255,255,255,.6)}" +
			".dsh-pet-bubble.below{bottom:auto;top:calc(100% + 12px)}" +
			".dsh-pet-bubble.below::after{top:auto;bottom:100%;border-top-color:transparent;border-bottom-color:rgba(255,255,255,.6)}" +
			/* 深色主题毛玻璃 */
			"body[data-ds-dark-theme] .dsh-pet-bubble{background:rgba(28,30,36,.6);border-color:rgba(255,255,255,.12)}" +
			"body[data-ds-dark-theme] .dsh-pet-bubble::after{border-top-color:rgba(28,30,36,.6)}" +
			"body[data-ds-dark-theme] .dsh-pet-bubble.below::after{border-bottom-color:rgba(28,30,36,.6)}" +
