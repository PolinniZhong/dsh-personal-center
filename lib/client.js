...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-inst-subtabs{display:flex;gap:2px;margin:0 0 4px}" +
			".dsh-inst-subtab{border:0;background:0 0;color:var(--dsw-alias-label-secondary);cursor:pointer;font-family:inherit;font-size:12px;line-height:20px;padding:2px 12px;border-radius:999px}" +
			".dsh-inst-subtab:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}" +
			".dsh-inst-subtab[data-active=true]{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);font-weight:600}" +
			".dsh-inst-section{display:none}" +
			".dsh-inst-section[data-active=true]{display:block;margin-top:8px}" +
			".dsh-inst-desc{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px;margin:0 0 10px}" +
			".dsh-inst-textarea{box-sizing:border-box;width:100%;min-height:200px;resize:vertical;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:10px 12px;font-family:inherit;font-size:13px;line-height:20px}" +
			".dsh-inst-textarea:focus{outline:none;border-color:var(--dsw-static-neutral-bluish-400)}" +
			".dsh-inst-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-top:8px}" +
			".dsh-inst-btn{cursor:pointer;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-module-platform);border-radius:8px;height:24px;padding:0 12px;font-size:11px;line-height:18px;font-family:inherit;color:var(--dsw-alias-label-primary);display:inline-flex;align-items:center;justify-content:center}" +
			".dsh-inst-btn:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}" +
			".dsh-inst-btn:disabled{opacity:.5;cursor:default}" +
			".dsh-inst-btn-primary{background:var(--dsw-alias-button-primary-fill);border-color:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground)}" +
			".dsh-inst-btn-primary:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}" +
			".dsh-inst-saved{color:var(--dsw-alias-state-success-primary);font-size:11px;line-height:18px;opacity:0;transition:opacity .3s}" +
			".dsh-inst-saved.show{opacity:1}" +
			/* 工作区列表 */
			".dsh-inst-ws{display:flex;flex-direction:column;gap:10px}" +
			/* 工作区条目:无外层描边框(参考全局配置:输入框自带边框 + 下方操作),当前用路径主题色区分 */
			".dsh-inst-ws-item{padding:10px 0}" +
			".dsh-inst-ws-item[data-current=true] .dsh-inst-ws-path{color:var(--dsw-alias-state-business-primary)}" +
			".dsh-inst-ws-head{display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap}" +
			".dsh-inst-ws-path{font-size:12.5px;font-weight:600;color:var(--dsw-alias-label-primary);min-width:0;flex:1;word-break:break-all;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;cursor:pointer}" +
			".dsh-inst-ws-path:hover{color:var(--dsw-alias-state-business-primary)}" +
			/* 状态标签:无胶囊背景、清淡;当前工作区 = 主题色文字,已配置 = 中性色 */
			".dsh-inst-ws-tag{font-size:10.5px;color:var(--dsw-alias-label-tertiary);white-space:nowrap;margin-left:4px}" +
			".dsh-inst-ws-tag[data-current=true]{color:var(--dsw-alias-state-business-primary);font-weight:600}" +
			".dsh-inst-ws-tag[data-configured=true]{color:var(--dsw-alias-label-secondary)}" +
			".dsh-inst-ws textarea{box-sizing:border-box;width:100%;min-height:56px;resize:vertical;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:8px 10px;font-family:inherit;font-size:12.5px;line-height:18px}" +
			".dsh-inst-ws textarea:focus{outline:none;border-color:var(--dsw-static-neutral-bluish-400)}" +
			".dsh-inst-ws-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-top:6px}" +
			/* 指令应用示例(内联在操作行左侧,点击展开紧凑预览) */
			".dsh-inst-preview-tag{border:0;background:0 0;color:var(--dsw-alias-label-tertiary);cursor:pointer;font-family:inherit;font-size:11px;line-height:18px;padding:0;margin-right:auto;display:inline-flex;align-items:center;gap:3px}" +
			".dsh-inst-preview-tag:hover{color:var(--dsw-alias-label-primary)}" +
			".dsh-inst-preview-tag svg{flex:none}" +
			".dsh-inst-preview-inline{border-top:1px solid var(--dsw-alias-border-l2);margin-top:8px;padding-top:8px}" +
			".dsh-inst-preview-title{font-size:11px;color:var(--dsw-alias-label-tertiary);margin-bottom:4px;display:flex;gap:8px;align-items:center;flex-wrap:wrap}" +
			".dsh-inst-preview-tokens{margin-left:auto;color:var(--dsw-alias-label-tertiary)}" +
			".dsh-inst-preview-box{font-size:11px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;line-height:1.6;white-space:pre-wrap;color:var(--dsw-alias-label-secondary);max-height:80px;overflow-y:auto;margin-bottom:4px}" +
			".dsh-inst-preview-empty{color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:18px;margin-bottom:4px}" +
			".dsh-inst-preview-ws{font-size:11px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;line-height:1.6;white-space:pre-wrap;color:var(--dsw-alias-state-business-primary);max-height:80px;overflow-y:auto}" +
			/* 工作区选择器:点击即下拉(自定义,替代 datalist);箭头内嵌输入框右端 */
			".dsh-inst-ws-picker{position:relative;margin-top:12px;min-width:0}" +
			".dsh-inst-ws-picker input{box-sizing:border-box;width:100%;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;height:28px;padding:0 30px 0 10px;font-size:12px;font-family:inherit;outline:none}" +
			".dsh-inst-ws-picker input:focus{border-color:var(--dsw-static-neutral-bluish-400)}" +
			".dsh-inst-ws-caret{position:absolute;top:50%;right:6px;transform:translateY(-50%);border:0;background:none;color:var(--dsw-alias-label-tertiary);cursor:pointer;padding:4px;display:inline-flex;align-items:center;justify-content:center}" +
			".dsh-inst-ws-caret:hover{color:var(--dsw-alias-label-primary)}" +
			".dsh-inst-ws-menu{position:absolute;top:calc(100% + 4px);left:0;right:0;z-index:50;background:var(--dsw-alias-bg-module-platform);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;max-height:200px;overflow-y:auto;box-shadow:var(--dsw-shadow-lv3);padding:4px}" +
			".dsh-inst-ws-opt{display:block;width:100%;text-align:left;background:none;border:none;cursor:pointer;color:var(--dsw-alias-label-primary);font-size:12px;line-height:18px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;padding:7px 8px;border-radius:6px;word-break:break-all}" +
			".dsh-inst-ws-opt:hover{background:var(--dsw-alias-interactive-bg-hover)}" +
			".dsh-inst-ws-pick{display:flex;gap:6px;align-items:center;flex-wrap:wrap}" +
			".dsh-inst-empty{padding:14px;text-align:center;color:var(--dsw-alias-label-tertiary);font-size:12px;border:1px dashed var(--dsw-alias-border-l2);border-radius:10px}" +
			/* 模板库:每个模板一个独立描边框(像绘画库,一眼区分) */
			".dsh-inst-tpl-list{display:flex;flex-direction:column}" +
			".dsh-inst-tpl-row{border:1px solid var(--dsw-alias-border-l2);border-radius:10px;align-items:center;gap:10px;padding:12px 14px;display:flex;margin-bottom:8px}" +
			".dsh-inst-tpl-row:last-child{margin-bottom:0}" +
			".dsh-inst-tpl-row-text{flex-direction:column;flex:1;gap:3px;min-width:0;display:flex}" +
			".dsh-inst-tpl-row-head{display:flex;align-items:center;gap:6px}" +
			".dsh-inst-tpl-row-name{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:400;line-height:22px}" +
			".dsh-inst-tpl-row-desc{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px}" +
			".dsh-inst-tpl-row-actions{display:flex;gap:6px;align-items:center;flex:none}" +
			".dsh-inst-tpl-badge{font-size:10px;padding:0 6px;border-radius:8px;background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-tertiary)}" +
			".dsh-inst-tpl-text{display:none;font-size:11.5px;color:var(--dsw-alias-label-secondary);line-height:1.6;white-space:pre-wrap;background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:8px 10px;max-height:96px;overflow-y:auto;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}" +
			".dsh-inst-tpl-text[data-open=true]{display:block;margin-top:4px}" +
			".dsh-inst-tpl-save{display:flex;gap:8px;align-items:center;margin-top:14px;flex-wrap:wrap}" +
			".dsh-inst-tpl-save input{box-sizing:border-box;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;height:26px;padding:0 10px;font-size:12px;font-family:inherit;outline:none;min-width:140px}" +
			".dsh-inst-tpl-save input:focus{border-color:var(--dsw-static-neutral-bluish-400)}" +
			/* 条件规则(v0.8):规则条目无外层边框,条目间细线分隔 */
			".dsh-inst-rules{margin-top:4px}" +
			".dsh-inst-rule{padding:12px 0}" +
			".dsh-inst-rule + .dsh-inst-rule{border-top:1px solid var(--dsw-alias-border-l2)}" +
			".dsh-inst-rule-conds{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:6px}" +
			".dsh-inst-rule-conds input{box-sizing:border-box;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;height:26px;padding:0 10px;font-size:12px;font-family:inherit;outline:none;min-width:110px;flex:1}" +
			".dsh-inst-rule-conds input:focus{border-color:var(--dsw-static-neutral-bluish-400)}" +
			".dsh-inst-rule-conds input:first-child{flex:2.2}" +
			".dsh-inst-rule textarea{box-sizing:border-box;width:100%;min-height:48px;resize:vertical;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:8px 10px;font-family:inherit;font-size:12.5px;line-height:18px}" +
			".dsh-inst-rule textarea:focus{outline:none;border-color:var(--dsw-static-neutral-bluish-400)}" +
			".dsh-inst-rule-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-top:6px}" +
			".dsh-inst-rules-add{margin-top:10px}" +
			/* 模板导入导出(v0.8) */
			".dsh-inst-tpl-io{display:flex;gap:6px;align-items:center;margin-top:12px;flex-wrap:wrap}" +
			".dsh-inst-import-panel{border-top:1px solid var(--dsw-alias-border-l2);margin-top:10px;padding-top:10px}" +
			".dsh-inst-import-panel textarea{box-sizing:border-box;width:100%;min-height:80px;resize:vertical;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:8px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11.5px;line-height:1.6}" +
			".dsh-inst-import-panel textarea:focus{outline:none;border-color:var(--dsw-static-neutral-bluish-400)}" +
			".dsh-inst-import-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-top:8px}" +
			".dsh-inst-import-actions .dsh-inst-saved{margin-right:auto}" +
			/* 注入预览(与上层输入框一致的描边;默认只显示工作区指令,全局折叠) */
			".dsh-inst-preview{border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:10px 12px;margin-top:14px;background:transparent}" +
			".dsh-inst-preview-title{font-size:11.5px;color:var(--dsw-alias-label-tertiary);margin-bottom:5px;display:flex;gap:8px;align-items:center;flex-wrap:wrap}" +
			".dsh-inst-preview-title b{color:var(--dsw-alias-state-business-primary);font-weight:600}" +
			".dsh-inst-preview-tokens{margin-left:auto;color:var(--dsw-alias-label-tertiary)}" +
			".dsh-inst-preview-box{font-size:11.5px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;line-height:1.6;white-space:pre-wrap;color:var(--dsw-alias-label-secondary);max-height:64px;overflow-y:auto;margin-bottom:6px}" +
			".dsh-inst-preview-empty{color:var(--dsw-alias-label-tertiary);font-size:11.5px;line-height:18px;margin-bottom:6px}" +
			".dsh-inst-preview-toggle{border:0;background:0 0;color:var(--dsw-alias-label-tertiary);cursor:pointer;font-family:inherit;font-size:11px;line-height:18px;padding:0;margin-bottom:4px}" +
			".dsh-inst-preview-toggle:hover{color:var(--dsw-alias-label-primary)}" +
			".dsh-inst-preview-global{font-size:11.5px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;line-height:1.6;white-space:pre-wrap;color:var(--dsw-alias-label-tertiary);max-height:110px;overflow-y:auto;border-top:1px solid var(--dsw-alias-border-l2);padding-top:6px;margin-bottom:6px}" +
			".dsh-inst-preview-note{font-size:11px;color:var(--dsw-alias-label-tertiary);margin:0}" +
			...
			".dsh-pet-status{position:fixed;z-index:2147483001;width:320px;max-width:calc(100vw - 24px);background:rgba(255,255,255,.45);-webkit-backdrop-filter:blur(7px) saturate(.75);backdrop-filter:blur(7px) saturate(.75);color:var(--dsw-alias-label-primary);border:1px solid rgba(255,255,255,.55);border-radius:16px;box-shadow:0 4px 12px rgba(0,0,0,.04);overflow:hidden;display:none}" +
			...
			".dsh-pet-status-sum b{margin-left:3px;font-size:14px;font-weight:600;line-height:20px;font-variant-numeric:tabular-nums}" +
			...
			".dsh-pet-status-list{max-height:178px;overflow-y:auto;padding:6px 10px 12px}" +
			".dsh-pet-status-item{display:flex;align-items:center;gap:8px;padding:7px 8px;border-radius:8px;font-size:12px;line-height:18px}" +
		...
		const dictionaries = {
			zh: {
				...
				"ciSavedShort": "✓ 已保存",
				"ciRules": "条件规则",
				"ciRulesDesc": "按模型 / 时段自动追加指令;命中时注入(优先级:条件 > 工作区 > 全局)。",
				"ciRuleModel": "模型(可空 = 任意;如 provider::model 或仅模型名)",
				"ciRuleTimeStart": "起(如 09:00)",
				"ciRuleTimeEnd": "止(如 18:00)",
				"ciRuleText": "该条件下追加的指令",
				"ciRuleAdd": "＋ 添加规则",
				"ciRuleDelete": "删除",
				"ciRuleEmpty": "未配置条件规则",
				"ciTplExport": "导出 JSON",
				"ciTplImport": "导入",
				"ciTplShare": "分享",
				"ciTplImportHint": "粘贴模板 JSON(数组或 {templates:[…]};同名将跳过)",
				"ciTplImportDo": "导入",
				"ciTplImported": "已导入 {{n}} 个,跳过 {{m}} 个",
				"ciTplImportBad": "JSON 解析失败或格式不支持"
			},
			en: {
				"nav": "Personal",
				"heading": "Personal",
				...
				"ciSavedShort": "✓ Saved",
				"ciRules": "Condition rules",
				"ciRulesDesc": "Append instructions by model / time window; injected when matched (priority: rules > workspace > global).",
				"ciRuleModel": "Model (empty = any; e.g. provider::model or model name)",
				"ciRuleTimeStart": "From (e.g. 09:00)",
				"ciRuleTimeEnd": "To (e.g. 18:00)",
				"ciRuleText": "Instructions appended when matched",
				"ciRuleAdd": "＋ Add rule",
				"ciRuleDelete": "Delete",
				"ciRuleEmpty": "No condition rules configured",
				"ciTplExport": "Export JSON",
				"ciTplImport": "Import",
				"ciTplShare": "Share",
				"ciTplImportHint": "Paste template JSON (array or {templates:[…]}; same names are skipped)",
				"ciTplImportDo": "Import",
				"ciTplImported": "Imported {{n}}, skipped {{m}}",
				"ciTplImportBad": "Invalid JSON or unsupported format"
			}
		};
		//#endregion
		...
		function PersonalizationTab({ t }) {
			...
			const [ruleDrafts, setRuleDrafts] = react.useState({});
			const [ruleSaving, setRuleSaving] = react.useState(false);
			const [ruleSavedKey, setRuleSavedKey] = react.useState("");
			...
			react.useEffect(() => {
				...
				Promise.all([
					...
					if (ins && ins.ok !== false) {
						...
						if (Array.isArray(ins.rules)) {
							setRules(ins.rules);
							const rd = {};
							for (const r of ins.rules) rd[r.id] = { model: r.model || "", timeStart: r.timeStart || "", timeEnd: r.timeEnd || "", text: r.text };
							setRuleDrafts(rd);
						}
			...
			// 规则预览文本(客户端无运行时模型/时段信号,展示规则配置;实际命中由宿主按运行时条件决定)
			const rulesPreviewText = rules.map((r) => {
				const cond = (r.model ? r.model : "任意模型") + (r.timeStart || r.timeEnd ? " · " + (r.timeStart || "…") + "–" + (r.timeEnd || "…") : "");
				return "【条件:" + cond + "】\n" + (r.text || "");
			}).join("\n\n");
			const mergedTokens = Math.ceil((merged + "\n\n" + rulesPreviewText).length / 4);

			/** 持久化分层配置(工作区 + 模板,整体保存)。 */
			/** 全量持久化(workspaces + 自定义模板 + 规则)。templates 过滤内置(内置由宿主合并,不重复存)。 */
			const persistAll = (nextWs, nextTpl, nextRules) => fetch("/personal-center/instructions", {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({
					workspaces: nextWs,
					templates: (nextTpl ?? templates).filter((t) => !String(t.id || "").startsWith("builtin-")),
					rules: nextRules ?? rules
				})
			}).then((r) => r.json()).then((d) => {
				if (!d || d.ok === false) throw new Error(d && d.error ? d.error : "failed");
			});
			const persistLayer = (nextWs, nextTpl) => persistAll(nextWs, nextTpl, rules);
			...
			const saveRules = (nextRules) => {
				setRuleSaving(true);
				persistAll(wsMap, templates, nextRules).then(() => {
					setRules(nextRules);
					setRuleSaving(false);
				}).catch(() => setRuleSaving(false));
			};
			const onSaveRule = (id) => {
				const draft = ruleDrafts[id];
				if (!draft || !draft.text || draft.text.trim() === "") return;
				const next = rules.map((r) => (r.id === id ? { ...r, model: draft.model || "", timeStart: draft.timeStart || "", timeEnd: draft.timeEnd || "", text: draft.text } : r));
				saveRules(next);
				setRuleSavedKey(id);
				setTimeout(() => setRuleSavedKey((k) => (k === id ? "" : k)), 1500);
			};
			...
			const onAddRule = () => {
				...
				setRuleDrafts({ ...ruleDrafts, [id]: { model: "", timeStart: "", timeEnd: "", text: "" } });
				saveRules([...rules, { id, model: "", timeStart: "", timeEnd: "", text: "" }]);
			};
			...
			const onExportTpl = () => {
				...
				try {
					...
					a.href = url;
					a.download = "dsh-personal-center-templates.json";
			...
			const onImportTpl = () => {
				let parsed;
				try { parsed = JSON.parse(importText); } catch { setImportResult(t("ciTplImportBad")); return; }
				const arr = Array.isArray(parsed) ? parsed : (parsed && Array.isArray(parsed.templates) ? parsed.templates : null);
				if (!arr) { setImportResult(t("ciTplImportBad")); return; }
				const valid = arr.filter((x) => x && typeof x === "object" && typeof x.text === "string" && x.text.trim() !== "");
				const existing = new Set(templates.map((x) => x.name));
				let added = 0;
				let skipped = 0;
				const merged = [...templates];
				for (const x of valid) {
					if (existing.has(x.name)) { skipped++; continue; }
					merged.push({ id: "custom-" + Date.now().toString(36) + added, name: String(x.name || "导入模板"), desc: String(x.desc || ""), text: x.text });
					existing.add(x.name);
					added++;
				}
				persistAll(wsMap, merged, rules).then(() => {
					setTemplates(merged);
					setImportResult(t("ciTplImported").replace("{{n}}", String(added)).replace("{{m}}", String(skipped)));
				}).catch(() => setImportResult(t("ciTplImportBad")));
			};
			const onShareTpl = (tpl) => copyTextFallback("【" + tpl.name + "】\n" + (tpl.desc ? tpl.desc + "\n" : "") + tpl.text);
			...
			return react.createElement("div", { className: "dsh-pc-panel" },
				// 子 tab(胶囊)
				react.createElement("div", { className: "dsh-inst-subtabs", role: "tablist" },
					subtabs.map(([id, label]) => react.createElement("button", {
						key: id,
						role: "tab",
						"data-active": sub === id,
						"aria-selected": sub === id,
						className: "dsh-inst-subtab",
						onClick: () => setSub(id)
					}, label))
				),
				// ── 全局指令 ──
				react.createElement("div", { className: "dsh-inst-section", "data-active": sub === "global" },
					react.createElement("p", { className: "dsh-inst-desc" }, t("ciGlobalDesc")),
					react.createElement("textarea", {
						className: "dsh-inst-textarea",
						value: draft,
						spellCheck: false,
					...
					react.createElement("div", { className: "dsh-inst-actions" },
						draft !== "" && btn("dsh-ci-button dsh-ci-secondary", t("clear"), onClearGlobal),
						btn("dsh-ci-button dsh-ci-primary", state === "saved" ? t("saved") : state === "failed" ? t("saveFailed") : t("save"), onSaveGlobal, state === "saving" || !dirty)
					),
					// ── 条件规则(v0.8:全局维度,按模型/时段自动追加)──
					react.createElement("div", { className: "dsh-inst-rules" },
						react.createElement("h3", { className: "dsh-pc-group" }, t("ciRules")),
						react.createElement("p", { className: "dsh-inst-desc" }, t("ciRulesDesc")),
						rules.length === 0
							? react.createElement("div", { className: "dsh-inst-empty" }, t("ciRuleEmpty"))
							: react.createElement("div", { className: "dsh-inst-rules-list" },
								rules.map((rule) => {
									const draft = ruleDrafts[rule.id] || { model: "", timeStart: "", timeEnd: "", text: "" };
									return react.createElement("div", { key: rule.id, className: "dsh-inst-rule" },
										react.createElement("div", { className: "dsh-inst-rule-conds" },
											react.createElement("input", {
												value: draft.model,
												placeholder: t("ciRuleModel"),
												spellCheck: false,
												onChange: (e) => setRuleDrafts({ ...ruleDrafts, [rule.id]: { ...draft, model: e.target.value } })
											}),
											react.createElement("input", {
												value: draft.timeStart,
												placeholder: t("ciRuleTimeStart"),
												spellCheck: false,
												onChange: (e) => setRuleDrafts({ ...ruleDrafts, [rule.id]: { ...draft, timeStart: e.target.value } })
											}),
											react.createElement("input", {
												value: draft.timeEnd,
												placeholder: t("ciRuleTimeEnd"),
												spellCheck: false,
												onChange: (e) => setRuleDrafts({ ...ruleDrafts, [rule.id]: { ...draft, timeEnd: e.target.value } })
											})
										),
										react.createElement("textarea", {
											value: draft.text,
											placeholder: t("ciRuleText"),
											spellCheck: false,
											onChange: (e) => setRuleDrafts({ ...ruleDrafts, [rule.id]: { ...draft, text: e.target.value } })
										}),
										react.createElement("div", { className: "dsh-inst-rule-actions" },
											react.createElement("span", { className: "dsh-inst-saved" + (ruleSavedKey === rule.id ? " show" : "") }, t("ciSavedShort")),
											btn("dsh-inst-btn", t("ciRuleDelete"), () => onDeleteRule(rule.id)),
											btn("dsh-inst-btn dsh-inst-btn-primary", t("ciRuleSave"), () => onSaveRule(rule.id), ruleSaving || !draft.text.trim())
										)
									);
								})
							),
						btn("dsh-inst-btn dsh-inst-btn-primary dsh-inst-rules-add", t("ciRuleAdd"), onAddRule)
					)
				),
				// ── 按工作区(含模板库:添加工作区后直接选模板,无需切换 tab)──
				react.createElement("div", { className: "dsh-inst-section", "data-active": sub === "ws" },
					react.createElement("p", { className: "dsh-inst-desc" }, t("ciWsDesc")),
					loading
						? react.createElement("div", { className: "dsh-inst-empty" }, "…")
						: wsKeys.length === 0
							? react.createElement("div", { className: "dsh-inst-empty" }, t("ciWsEmpty"))
							: react.createElement("div", { className: "dsh-inst-ws" },
								wsKeys.map((key) => {
									...
									return react.createElement("div", { key, className: "dsh-inst-ws-item", "data-current": isCurrent },
										react.createElement("div", { className: "dsh-inst-ws-head" },
											react.createElement("span", { className: "dsh-inst-ws-path", title: key, onClick: togglePath }, expanded ? key : short),
											!isCurrent && react.createElement("span", { className: "dsh-inst-ws-tag" }, t("ciWsFallback")),
											(wsMap[key] || "") !== "" && react.createElement("span", { className: "dsh-inst-ws-tag", "data-configured": true }, t("ciWsConfigured"))
										),
										react.createElement("textarea", {
											value: wsDrafts[key] ?? "",
											spellCheck: false,
										...
										isCurrent && openWs && react.createElement("div", { className: "dsh-inst-preview-inline" },
											react.createElement("div", { className: "dsh-inst-preview-title" },
												react.createElement("span", null, t("ciPreviewTag") + " · " + (currentWs ? currentWs.split("/").pop() : "")),
												react.createElement("span", { className: "dsh-inst-preview-tokens" }, t("ciPreviewTokens").replace("{{n}}", String(mergedTokens)))
											),
											(draft || "").trim() !== ""
												? react.createElement("div", { className: "dsh-inst-preview-box" }, draft)
												: react.createElement("div", { className: "dsh-inst-preview-empty" }, t("ciGlobalEmpty")),
											currentWsText && react.createElement("div", { className: "dsh-inst-preview-ws" }, currentWsText),
											rules.length > 0 && react.createElement("div", { className: "dsh-inst-preview-ws", style: { color: "var(--dsw-alias-label-tertiary)", marginTop: 4 } }, rulesPreviewText)
										),
										react.createElement("div", { className: "dsh-inst-ws-actions" },
											isCurrent && btn("dsh-inst-preview-tag", t("ciPreviewTag") + (openWs ? " ▴" : " ▾"), () => setOpenWs(!openWs)),
											react.createElement("span", { className: "dsh-inst-saved" + (wsSavedKey === key ? " show" : "") }, t("ciSavedShort")),
											btn("dsh-inst-btn", t("clear"), () => onClearWs(key)),
											btn("dsh-inst-btn dsh-inst-btn-primary", t("save"), () => onSaveWs(key), wsSaving)
										)
									);
								})
							),
					// 工作区选择器:点击输入框/箭头即下拉;点外部(document mousedown + contains)立即关闭
					react.createElement("div", { className: "dsh-inst-ws-picker", ref: pickerRef },
						react.createElement("input", {
							value: addPath,
						...
						react.createElement("span", { className: "dsh-inst-ws-caret", onClick: () => setWsOpen(!wsOpen) },
						...
						wsOpen && react.createElement("div", { className: "dsh-inst-ws-menu" },
							discovered.map((p) => react.createElement("button", {
								key: p,
								className: "dsh-inst-ws-opt",
								onClick: () => { setAddPath(p); setWsOpen(false); ensureWs(p); }
							}, p))
						)
					),
					// 指令模板库(并入本子区:添加/配置工作区时下方直接选模板)
					react.createElement("div", { className: "dsh-inst-tpl-in-ws" },
						react.createElement("h3", { className: "dsh-pc-group" }, t("ciSubTpl")),
						react.createElement("p", { className: "dsh-inst-desc" }, t("ciTplDesc")),
						react.createElement("div", { className: "dsh-inst-tpl-list" },
							templates.map((tpl) => {
								const isBuiltin = String(tpl.id || "").startsWith("builtin-");
								const open = openTpls.has(tpl.id);
								return react.createElement("div", { key: tpl.id, className: "dsh-inst-tpl-row" },
									react.createElement("div", { className: "dsh-inst-tpl-row-text" },
										react.createElement("div", { className: "dsh-inst-tpl-row-head" },
											react.createElement("span", { className: "dsh-inst-tpl-row-name" }, tpl.name || "—"),
											!isBuiltin && react.createElement("span", { className: "dsh-inst-tpl-badge" }, t("ciTplCustom"))
										),
										react.createElement("div", { className: "dsh-inst-tpl-row-desc" }, tpl.desc || ""),
										react.createElement("div", { className: "dsh-inst-tpl-text", "data-open": open }, tpl.text || "")
									),
									react.createElement("div", { className: "dsh-inst-tpl-row-actions" },
										btn("dsh-inst-btn", open ? t("ciTplCollapse") : t("ciTplView"), () => {
											const next = new Set(openTpls);
											if (next.has(tpl.id)) next.delete(tpl.id); else next.add(tpl.id);
											setOpenTpls(next);
										}),
										btn("dsh-inst-btn dsh-inst-btn-primary", t("ciTplApplyWs"), () => onApplyTpl(tpl.text, "ws")),
										btn("dsh-inst-btn", t("ciTplShare"), () => onShareTpl(tpl)),
										!isBuiltin && btn("dsh-inst-btn", t("ciTplDelete"), () => onDeleteTpl(tpl.id))
									)
								);
							})
						),
						// 存为模板
						react.createElement("div", { className: "dsh-inst-tpl-save" },
							react.createElement("input", {
								value: tplName,
								placeholder: t("ciTplName"),
								onChange: (e) => setTplName(e.target.value)
							}),
							btn("dsh-inst-btn dsh-inst-btn-primary", t("ciTplSaveAs"), onSaveAsTpl, !draft.trim()),
							tplSaved && react.createElement("span", { className: "dsh-inst-saved show" }, t("saved"))
						),
						// 导入导出(v0.8)
						react.createElement("div", { className: "dsh-inst-tpl-io" },
							btn("dsh-inst-btn", t("ciTplExport"), onExportTpl),
							btn("dsh-inst-btn", t("ciTplImport"), () => setImportOpen(!importOpen)),
							importOpen && react.createElement("div", { className: "dsh-inst-import-panel" },
								react.createElement("textarea", {
									value: importText,
									placeholder: t("ciTplImportHint"),
									spellCheck: false,
									onChange: (e) => setImportText(e.target.value)
								}),
								react.createElement("div", { className: "dsh-inst-import-actions" },
									importResult !== "" && react.createElement("span", { className: "dsh-inst-saved show" }, importResult),
									btn("dsh-inst-btn dsh-inst-btn-primary", t("ciTplImportDo"), onImportTpl, !importText.trim())
								)
							)
						)
					)
		...
		class PetWidget {
			...
			_endAction() {
				this._actionTimer = null;
				this._acting = false;
				// 静止 = 睡觉(桌面端浮层待机统一显示 dozing,动的时候才按情绪播动画)
				this._showImg("dozing", "idle");
			}
			...
			_switchEmotion(emotion) {
				...
				// 动作中:显示新情绪动画;静止:显示睡觉(dozing)
				this._showImg(this._acting ? emotion : "dozing", this._acting ? "animations" : "idle");
				this._containerEl.classList.remove("emotion-" + prev);
				this._containerEl.classList.add("emotion-" + emotion);
				// 情绪变化时播放一次对应动画(让变化可见),播完回待机(睡觉);逗弄中则沿用正在播的动画
				this.playAction();
			}
		}
		...
		class PetStatusPanel {
			...
			_build() {
				...
				this.el = el;
				this.sumEls = {
					running: el.querySelector(".running b"),
					failed: el.querySelector(".failed b"),
					done: el.querySelector(".done b")
				};
			...
			_project() {
				...
				for (const id of snap.ids || []) {
					...
					if (!entry) continue;
					// 空白会话(无内容)不展示:其 displayTitle 会兜底成项目目录名(如「03_日常对话」),
					// 观感像把项目当成对话,过滤之。
					...
					else if (entry.running) status = "running";
			...
			_render() {
				...
				this.sumEls.running.textContent = run;
				this.sumEls.failed.textContent = fail;
				this.sumEls.done.textContent = done;
				const t = this.t;
				if (rows.length === 0) {
					this.listEl.innerHTML = '<div class="dsh-pet-status-empty">' + t("petStatusEmpty") + "</div>";
				} else {
					// 列表最多显示 5 条(其余滚动);摘要计数仍按全量统计
					this.listEl.innerHTML = rows.slice(0, 5).map((r) =>
						'<div class="dsh-pet-status-item"><span class="dsh-pet-status-dot" style="background:' +
				...
				// 状态分布变化 → 气泡(3.5s)+ 情绪映射
				const key = fail + ":" + run + ":" + done;
				if (key !== this.lastKey) {
					this.lastKey = key;
					this._announce(fail, run, done);
				}
				this.position();
			}

			_announce(fail, run, done) {
				// 状态已在面板列表内完整展示,不再额外弹状态气泡(避免与数据气泡并存成"两个气泡")。
				// 仅做情绪映射(规格:失败 > 运行中 > 完成 > 空闲):着急/忙碌/开心/打盹
				// 失败用 exhausted(焦头烂额),运行中用 busy,完成用 happy,空闲用 dozing
				this.pet.setEmotion(fail > 0 ? "exhausted" : run > 0 ? "busy" : done > 0 ? "happy" : "dozing");
			}
