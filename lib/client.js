...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PetPanel({ t, petCtl }) {
			...
			// 只有「已启用」的卡片走实时数据(情绪/统计);未启用的卡片保持静态,不随数据变化(省资源)。
			const petCard = (id) => {
				const isActive = curSkin === id;
				const isEnabled = cfg.enabled === id;
				const cardEmotion = isEnabled ? emotion : null;
				const cardEmoKey = cardEmotion ? PET_EMOTION_LABEL_KEYS[cardEmotion] : null;
				const asset = cardEmotion
				...
				return react.createElement("div", {
					...
					react.createElement("div", { className: "dsh-pc-pet-emo-bubble" },
						cardEmotion ? react.createElement("img", { className: "dsh-pc-pet-emo-img", src: PET_ASSET_BASE + id + "/idle/" + cardEmotion + ".webp", alt: "" }) : null,
						react.createElement("span", { className: "dsh-pc-pet-emo-name" }, cardEmoKey ? t(cardEmoKey) : t("petStatusOff"))
					),
