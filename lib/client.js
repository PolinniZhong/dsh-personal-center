...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PersonalizationTab({ t }) {
				...
				Promise.all([
					...
					if (ins && ins.ok !== false) {
						...
						if (Array.isArray(ins.rules)) {
							setRules(ins.rules);
							const rd = {};
							for (const r of ins.rules) rd[r.id] = { timeStart: r.timeStart || "", timeEnd: r.timeEnd || "", text: r.text };
							setRuleDrafts(rd);
						}
			...
			// 规则预览文本(客户端无运行时时段信号,展示规则配置;实际命中由宿主按运行时时间决定)
			const rulesPreviewText = rules.map((r) => {
				const cond = r.timeStart || r.timeEnd ? "时段 " + (r.timeStart || "…") + "–" + (r.timeEnd || "…") : "任意时段";
				return "【条件:" + cond + "】\n" + (r.text || "");
			}).join("\n\n");
			...
			const onSaveRule = (id) => {
				...
				const next = rules.map((r) => (r.id === id ? { ...r, timeStart: draft.timeStart || "", timeEnd: draft.timeEnd || "", text: draft.text } : r));
				saveRules(next);
				setRuleSavedKey(id);
			...
			const onAddRule = () => {
				...
				setRuleDrafts({ ...ruleDrafts, [id]: { timeStart: "", timeEnd: "", text: "" } });
				saveRules([...rules, { id, timeStart: "", timeEnd: "", text: "" }]);
			};
					...
					react.createElement("div", { className: "dsh-inst-rules" },
						...
						rules.length === 0
							...
							: react.createElement("div", { className: "dsh-inst-rules-list" },
								rules.map((rule) => {
									const draft = ruleDrafts[rule.id] || { timeStart: "", timeEnd: "", text: "" };
