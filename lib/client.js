...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-inst-ws-add{display:flex;align-items:center;gap:8px;margin-top:12px;flex-wrap:wrap}" +
			".dsh-inst-ws-add input{box-sizing:border-box;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;height:26px;padding:0 10px;font-size:12px;font-family:inherit;outline:none;flex:1;min-width:180px}" +
			".dsh-inst-ws-add input:focus{border-color:var(--dsw-static-neutral-bluish-400)}" +
			...
			/* 模板库:列表行(参考 Agent 预设布局:标题+描述 | 右侧操作,border-bottom 分隔) */
			".dsh-inst-tpl-list{display:flex;flex-direction:column}" +
			".dsh-inst-tpl-row{border-bottom:1px solid var(--dsw-alias-border-l2);align-items:center;gap:10px;padding:14px 2px;display:flex}" +
			".dsh-inst-tpl-row:last-child{border-bottom:none}" +
		...
		const dictionaries = {
			...
			en: {
				...
				"ciTplApplyWs": "Apply to current workspace",
				"ciTplView": "View",
		...
		function PersonalizationTab({ t }) {
			...
			const [addMode, setAddMode] = react.useState("pick");
			...
			react.useEffect(() => {
				...
				Promise.all([
					...
					if (cur && typeof cur.cwd === "string") setCurrentWs(cur.cwd);
			...
			const onAddWs = () => {
				...
				if (!path || wsMap[path] !== undefined) return;
				const next = { ...wsMap, [path]: "" };
				setWsDrafts({ ...wsDrafts, [path]: "" });
				setWsMap(next); // 先本地呈现
				saveWorkspaces(next);
				setAddPath("");
			};
			...
			return react.createElement("div", { className: "dsh-pc-panel" },
				...
				react.createElement("div", { className: "dsh-inst-section", "data-active": sub === "ws" },
					...
					// 添加工作区
					react.createElement("div", { className: "dsh-inst-ws-add" },
						addMode === "pick" && discovered.length > 0
							? react.createElement("select", {
								value: addPath,
								onChange: (e) => setAddPath(e.target.value)
							},
								react.createElement("option", { value: "" }, t("ciWsPathPlaceholder")),
								discovered.map((p) => react.createElement("option", { key: p, value: p }, p))
							)
						...
						btn("dsh-inst-btn", addMode === "pick" ? t("ciWsNew") : t("ciWsPick"), () => setAddMode(addMode === "pick" ? "new" : "pick")),
						btn("dsh-inst-btn dsh-inst-btn-primary", t("ciWsAdd"), onAddWs, !addPath.trim())
					),
							...
							templates.map((tpl) => {
								...
								return react.createElement("div", { key: tpl.id, className: "dsh-inst-tpl-row" },
									...
									react.createElement("div", { className: "dsh-inst-tpl-row-actions" },
										...
										btn("dsh-inst-btn", t("ciTplApplyGlobal"), () => onApplyTpl(tpl.text, "global")),
				...
				react.createElement("div", { className: "dsh-inst-preview" },
					...
					currentWsText
						? react.createElement("div", { className: "dsh-inst-preview-box" }, currentWsText)
						: react.createElement("div", { className: "dsh-inst-preview-empty" }, t("ciWsNoRule")),
					(draft || "").trim() !== "" && react.createElement("div", null,
						btn("dsh-inst-preview-toggle", openGlobal ? t("ciGlobalToggleOpen") : t("ciGlobalToggle"), () => setOpenGlobal(!openGlobal)),
						openGlobal && react.createElement("div", { className: "dsh-inst-preview-global" }, draft)
					),
					react.createElement("p", { className: "dsh-inst-preview-note" }, t("ciPreviewNote"))
				)
			);
		}
		//#endregion
