...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-pc-pet-switch-thumb{position:absolute;top:2px;left:2px;width:16px;height:16px;border-radius:50%;background:var(--dsw-alias-label-primary);transition:transform .2s ease}" +
		...
		const dictionaries = {
			zh: {
				...
				"petTip": "默认静止,不打扰工作。鼠标悬停或点击会按当前情绪做个小动作;拖住它可以移动到任意位置;右键可隐藏宠物。",
				"petStatusOn": "已启用",
				"petStatusOff": "未启用",
				"petSaved": "已保存",
			...
			en: {
				...
				"petTip": "Idle by default, never distracts. Hover or click for a small move based on current mood; drag to place it anywhere; right-click to hide.",
				"petStatusOn": "Enabled",
		...
		/** 应用配置(面板改动):enabled 开关 + opacity 档位。持久化由调用方负责。 */
		...
		function PetPanel({ t, petCtl }) {
			...
			react.useEffect(() => {
				...
				petCtl.getConfig().then((c) => {
					...
					// 宿主未重启/接口不可用:回退默认值,面板仍可打开
					if (alive) setCfg({ enabled: true, size: "small", position: "bottom-right", posOverride: null, opacity: 1 });
				});
				const sync = () => {
			...
			const emoAsset = emotion ? PET_ASSET_BASE + "idle/" + emotion + ".webp" : null;
			// 今日统计
			...
			return react.createElement("div", { className: "dsh-pc-panel" },
				react.createElement("div", { className: "dsh-pc-pet-card" },
					react.createElement("img", { className: "dsh-pc-pet-preview-img", src: emoAsset || PET_ASSET_BASE + "idle/happy.webp", alt: "" }),
