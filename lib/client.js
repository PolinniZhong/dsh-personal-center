...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PetPanel({ t, petCtl }) {
			...
			// 唤醒反馈:宠物从「未启用 → 启用」时,右侧情绪气泡播一次动画(用户知道被唤醒了)
			...
			react.useEffect(() => {
				...
				if (prev !== undefined && cur && cur !== prev) {
					...
					setWake({ [cur]: emo });
					const t = setTimeout(() => setWake({}), PET_ACTION_MS);
			...
			// 左边形象图在面板打开期间定时随机播放小表情动画(anim),代表"宠物是活的"。
			const petCard = (id) => {
				...
				// 右侧情绪气泡图:唤醒动画(启用瞬间) > 启用卡当前情绪静态图 > 无图(未启用)
				const bubbleImg = wake[id]
					? PET_ASSET_BASE + id + "/animations/" + wake[id] + ".webp"
					: (cardEmotion ? PET_ASSET_BASE + id + "/idle/" + cardEmotion + ".webp" : null);
				return react.createElement("div", {
					...
					react.createElement("div", { className: "dsh-pc-pet-emo-bubble" },
						bubbleImg ? react.createElement("img", { className: "dsh-pc-pet-emo-img", src: bubbleImg, alt: "" }) : null,
