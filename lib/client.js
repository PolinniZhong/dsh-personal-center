...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-pc-activity-toggle{display:inline-flex;gap:4px;padding:2px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;align-self:flex-start}" +
			".dsh-pc-activity-mode{border:0;background:0 0;color:var(--dsw-alias-label-tertiary);cursor:pointer;font-family:inherit;font-size:12px;line-height:20px;padding:2px 12px;border-radius:6px}" +
			".dsh-pc-activity-mode[data-active=true]{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}" +
			".dsh-pc-heat{display:flex;flex-direction:column;gap:4px}" +
			".dsh-pc-heat-grid{display:grid;gap:3px}" +
			".dsh-pc-heat-label{font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary);white-space:nowrap;overflow:hidden}" +
			".dsh-pc-heat-cell{width:11px;height:11px;border-radius:3px;background:var(--dsw-alias-interactive-bg-hover)}" +
		...
		//#region token activity(示例数据;接入统计后按天/周/累计归组)
		...
		function buildMockActivity() {
			...
			for (let w = 0; w < ACTIVITY_WEEKS; w++) {
				...
				for (let d = 0; d < 7; d++) {
					const idx = w * 7 + d;
					const recency = idx / totalDays; // 0 旧 → 1 新
					let level = 0;
					if (fracRand(idx + 1) < 0.3 + 0.55 * recency) {
						level = 1 + Math.floor(fracRand(idx + 7919) * 5);
					}
					days.push(level);
				}
		...
		/** 最近 12 个自然月 + 逐月累计(示例)。 */
		function buildMockMonthlyCumulative() {
			const now = new Date();
			const months = [];
			for (let i = 11; i >= 0; i--) {
			...
			let cum = 0;
			for (let i = 0; i < 12; i++) {
				cum += Math.round(120000 + fracRand(i * 31 + 7) * 420000);
				totals.push(cum);
			}
			return { months, totals };
		}

		const MOCK_ACTIVITY_WEEKS = buildMockActivity();
		const MOCK_MONTHLY = buildMockMonthlyCumulative();
		...
		function ActivityDaily({ t }) {
			const weeks = MOCK_ACTIVITY_WEEKS;
			const W = weeks.length;
			const gridStyle = { gridTemplateColumns: "repeat(" + W + ", 11px)" };
			const cells = [];
			for (let d = 0; d < 7; d++) {
				for (let w = 0; w < W; w++) {
		...
		/** 累计:最近 12 个月的累计柱状图。 */
		function ActivityCumulative({ t }) {
			const max = Math.max.apply(null, MOCK_MONTHLY.totals) || 1;
			return react.createElement("div", { className: "dsh-pc-cum" },
				MOCK_MONTHLY.months.map((m, i) => react.createElement("div", { key: m.year + "-" + m.month, className: "dsh-pc-cum-col" },
					react.createElement("div", {
						className: "dsh-pc-cum-bar",
						style: { height: Math.max(2, Math.round((MOCK_MONTHLY.totals[i] / max) * 78)) + "px" }
					}),
		...
		/** Token 活动容器:切换每日/每周/累计。 */
		function TokenActivity({ t }) {
			...
			return react.createElement("div", { className: "dsh-pc-activity" },
				...
				mode === "daily" ? react.createElement(ActivityDaily, { t })
					: mode === "weekly" ? react.createElement(ActivityWeekly, { t })
					: react.createElement(ActivityCumulative, { t })
			);
		}
		//#endregion
		...
		function ProfileTab({ t }) {
			...
			return react.createElement("div", { className: "dsh-pc-panel" },
				...
				react.createElement("h3", { className: "dsh-pc-group" }, t("activityTitle")),
				react.createElement(TokenActivity, { t }),
