...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			/* 默认静止:无常驻循环动画(参考 Codex 宠物规范:待机不打扰,动作只在逗弄时播放) */
		...
		const dictionaries = {
			zh: {
				...
				"petResetPos": "重置到右下角",
				"petIdleHint": "默认静止;鼠标悬停或点击会逗它做个小动作。拖住它可以移动到任意位置。",
				"petStatusOn": "已启用",
				"petStatusOff": "未启用",
				"petSaved": "已保存",
			...
			en: {
				...
				"petIdleHint": "Idle by default; hover or click to make a small move. Drag to place it anywhere.",
				"petStatusOn": "Enabled",
		...
		/**
		 * 黑鲸 PetWidget:默认静止待机(单帧静态表情),悬停/点击播放一次动作动画,
		...
		/** 应用配置(启动或面板改动)。透明度/尺寸/角落即时生效,持久化由调用方负责。 */
		function petApplyConfig(cfg) {
			...
			if (cfg.position && cfg.position !== prev.position) {
				petConfig.posOverride = null;
				petWidget.setPosition(cfg.position);
			}
		}
		...
		function petResize(size) {
			petApplyConfig({ size });
			petPersist({ size }).catch(() => {});
		}

		/** 重置到右下角(清掉拖拽位置)。 */
		function petResetPosition() {
			if (petWidget) petWidget.setPosition("bottom-right");
			petConfig.position = "bottom-right";
			petConfig.posOverride = null;
			petPersist({ position: "bottom-right", posOverride: null }).catch(() => {});
		}

		/**
		 * 桌面宠物配置面板:启用/尺寸/位置(四角+自定义)/透明度 + 今日情绪 + 阈值说明。
		...
		function PetPanel({ t, petCtl }) {
			...
			const emoAsset = PET_ASSET_BASE + "idle/" + (emotion || "happy") + ".webp";
			...
			return react.createElement("div", { className: "dsh-pc-panel" },
				...
				react.createElement("div", { className: "dsh-pc-pet-opts" },
					...
					react.createElement("div", { className: "dsh-pc-pet-opt" },
						...
						react.createElement("div", { className: "dsh-pc-pill-tabs" },
							["bottom-right", "bottom-left", "top-right", "top-left"].map((id) => pill(id, posLabel[id], !custom && cfg.position === id, () => update({ position: id })))
						)
					),
					// 拖拽位置:显示自定义状态 + 重置
					custom ? react.createElement("div", { className: "dsh-pc-pet-opt" },
							react.createElement("span", { className: "dsh-pc-pet-opt-label" }, t("petPosCustom")),
							react.createElement("button", { className: "dsh-ci-button dsh-ci-secondary", onClick: () => { petCtl.resetPosition(); setCfg((c) => ({ ...c, position: "bottom-right", posOverride: null })); } }, t("petResetPos"))
						) : null,
					react.createElement("div", { className: "dsh-pc-pet-opt" },
						react.createElement("label", { className: "dsh-pc-pet-toggle" },
							t("petOpacity"),
							react.createElement("input", { className: "dsh-pc-pet-range", type: "range", min: "0.3", max: "1", step: "0.05", value: cfg.opacity, onInput: (e) => update({ opacity: Number(e.target.value) }) }),
				...
				react.createElement("p", { className: "dsh-pc-pet-hint" }, t("petIdleHint")),
		...
		function apply(ctx) {
			...
			const petCtl = {
				// 乐观应用:先本地生效(透明度/尺寸即时反馈),再异步持久化;持久化失败静默
