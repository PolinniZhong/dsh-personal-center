...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			/* 宠物列表(黑鲸/蓝鲸各一张卡片,点击切换) */
			".dsh-pc-pet-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:12px}" +
			".dsh-pc-pet-pick{display:flex;flex-direction:column;align-items:center;gap:6px;padding:14px 12px 10px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;cursor:pointer;color:var(--dsw-alias-label-primary);font-family:inherit}" +
			".dsh-pc-pet-pick:hover{background:var(--dsw-alias-interactive-bg-hover)}" +
			".dsh-pc-pet-pick[data-active=true]{border-color:var(--dsw-static-deepseek-400);background:var(--dsw-alias-interactive-bg-hover)}" +
			".dsh-pc-pet-pick img{width:56px;height:56px;border-radius:50%;object-fit:contain;background:var(--dsw-alias-interactive-bg-hover);flex:none}" +
			".dsh-pc-pet-pick-name{font-size:13px;font-weight:500;line-height:20px}" +
			".dsh-pc-pet-pick-badge{font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary)}" +
			".dsh-pc-pet-pick[data-active=true] .dsh-pc-pet-pick-badge{color:var(--dsw-static-deepseek-400);font-weight:500}" +
		...
		let petConfig = { enabled: true, skin: "black-whale", size: "medium", position: "bottom-right", posOverride: null, opacity: 1 };
		...
		/** 应用配置(面板改动):enabled 开关 + skin 皮肤 + opacity 档位。持久化由调用方负责。 */
		function petApplyConfig(cfg) {
			...
			if (cfg.skin && cfg.skin !== prev.skin) petWidget.setSkin(cfg.skin);
		}

		/** 右键菜单「隐藏宠物」(乐观应用:先本地隐藏,再异步持久化,与面板开关一致)。 */
		function petHide() {
			petApplyConfig({ enabled: false });
			petPersist({ enabled: false }).catch(() => {});
		}
		...
		function PetPanel({ t, petCtl }) {
			...
			react.useEffect(() => {
				...
				petCtl.getConfig().then((c) => {
					...
					if (!alive) return;
					setServiceDown(true);
					setCfg({ enabled: true, skin: "black-whale", size: "small", position: "bottom-right", posOverride: null, opacity: 1 });
				});
				const sync = () => {
			...
			// 今日情绪气泡(按当前皮肤取素材)
			const curSkin = PET_SKINS.indexOf(cfg.skin) >= 0 ? cfg.skin : "black-whale";
			const emoKey = PET_EMOTION_LABEL_KEYS[emotion] || null;
			const emoAsset = emotion ? PET_ASSET_BASE + curSkin + "/idle/" + emotion + ".webp" : null;
			// 今日统计
			...
			// 宠物列表:黑鲸/蓝鲸各一张卡片,点击即切换皮肤
			const petPick = (id) => react.createElement("button", {
				className: "dsh-pc-pet-pick" + (curSkin === id ? " active" : ""),
				"data-active": curSkin === id,
				onClick: () => update({ skin: id })
			},
				react.createElement("img", { src: PET_ASSET_BASE + id + "/idle/happy.webp", alt: "" }),
				react.createElement("span", { className: "dsh-pc-pet-pick-name" }, id === "black-whale" ? t("petNameBlack") : t("petNameBlue")),
				react.createElement("span", { className: "dsh-pc-pet-pick-badge" }, curSkin === id ? t("petStatusOn") : t("petStatusOff"))
			);
			...
			return react.createElement("div", { className: "dsh-pc-panel" },
				...
				react.createElement("div", { className: "dsh-pc-pet-card" },
					react.createElement("img", { className: "dsh-pc-pet-preview-img", src: emoAsset || PET_ASSET_BASE + curSkin + "/idle/happy.webp", alt: "" }),
					react.createElement("div", { className: "dsh-pc-pet-info" },
						react.createElement("div", { className: "dsh-pc-pet-title-row" },
							react.createElement("span", { className: "dsh-pc-pet-title" }, t("petTitle")),
							// 悬停「桌面宠物」标题旁的 ℹ 弹提示(DSH 主题化 popover)
						...
						react.createElement("div", { className: "dsh-pc-pet-sub" },
					...
					react.createElement("div", { className: "dsh-pc-pet-emo-bubble" },
						emoAsset ? react.createElement("img", { className: "dsh-pc-pet-emo-img", src: emoAsset, alt: "" }) : null,
						react.createElement("span", { className: "dsh-pc-pet-emo-name" }, emoKey ? t(emoKey) : t("petStatusOff"))
					),
					react.createElement("label", { className: "dsh-pc-pet-switch", title: cfg.enabled ? t("petStatusOn") : t("petStatusOff") },
