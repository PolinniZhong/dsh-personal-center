...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-pet-container{position:fixed;z-index:2147483000;width:120px;height:120px;cursor:grab;user-select:none;-webkit-user-select:none;transition:opacity .3s ease}" +
			".dsh-pet-container.dragging{cursor:grabbing;z-index:2147483647}" +
			".dsh-pet-container.hidden{opacity:0;pointer-events:none}" +
			".dsh-pet-wrapper{width:100%;height:100%;position:relative;animation:dsh-pet-breathe 4s ease-in-out infinite}" +
			"@keyframes dsh-pet-breathe{0%,100%{transform:scale(1)}50%{transform:scale(1.02)}}" +
			...
			".dsh-pet-container.emotion-happy{animation:dsh-pet-float-bounce 3s ease-in-out infinite}" +
			"@keyframes dsh-pet-float-bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}" +
			".dsh-pet-container.emotion-busy{animation:dsh-pet-shake .3s linear infinite}" +
			"@keyframes dsh-pet-shake{0%,100%{transform:translateY(0) translateX(0)}25%{transform:translateY(-4px) translateX(-2px)}75%{transform:translateY(-4px) translateX(2px)}}" +
			".dsh-pet-container.emotion-exhausted{animation:dsh-pet-float-slow 5s ease-in-out infinite}" +
			"@keyframes dsh-pet-float-slow{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}}" +
			".dsh-pet-container.emotion-money-pain{animation:dsh-pet-tremble .5s ease-in-out infinite}" +
			"@keyframes dsh-pet-tremble{0%,100%{transform:translateY(0) rotate(0)}25%{transform:translateY(-4px) rotate(-2deg)}75%{transform:translateY(-4px) rotate(2deg)}}" +
			".dsh-pet-container.emotion-dozing{animation:dsh-pet-float-slowest 6s ease-in-out infinite}" +
			"@keyframes dsh-pet-float-slowest{0%,100%{transform:translateY(0)}50%{transform:translateY(-3px)}}" +
		...
		const PET_EMOTION_LABEL_KEYS = {
			happy: "petEmotionHappy",
			busy: "petEmotionBusy",
			exhausted: "petEmotionExhausted",
			"money-pain": "petEmotionMoneyPain",
			dozing: "petEmotionDozing"
		};
		/**
		...
		/** 黑鲸 PetWidget:浮层 + 情绪动画 + 点击气泡 + 拖拽(纯 DOM,零依赖)。 */
		class PetWidget {
			/**
			 * @param {Object} o
			 * @param {HTMLElement} o.container 挂载容器
			 * @param {string} o.position 角落:bottom-right 等
			 ...
			 * @param {(size: string) => void} o.onResize 右键菜单改尺寸
			...
			_buildMenu() {
				...
				sizeRow.className = "dsh-pet-menu-item";
				const sizeLabel = document.createElement("span");
				sizeLabel.textContent = t("petMenuSize");
				const sizes = document.createElement("span");
				sizes.className = "dsh-pet-menu-sizes";
				const entries = [["small", t("petMenuSizeSmall")], ["medium", t("petMenuSizeMedium")], ["large", t("petMenuSizeLarge")]];
				for (const [id, label] of entries) {
					const b = document.createElement("span");
					b.className = "dsh-pet-menu-size";
					b.dataset.size = id;
					b.textContent = label;
					b.addEventListener("click", (e) => {
						e.stopPropagation();
						this._closeMenu();
						if (this.options.onResize) this.options.onResize(id);
					});
					sizes.appendChild(b);
				}
			...
			_openMenu(x, y) {
				...
				const children = this._menuEl.children || [];
				for (const row of children) {
					const sizeBtns = row.querySelectorAll ? row.querySelectorAll(".dsh-pet-menu-size") : [];
					for (const b of sizeBtns) b.dataset.active = String(b.dataset.size === this.options.size);
				}
			...
			_switchEmotion(emotion) {
				...
				if (prev && this._imgEls[prev]) {
					const p = this._imgEls[prev];
					p.removeAttribute("src");
					this._imgLoaded[prev] = false;
					p.classList.remove("active");
				}
				const img = this._imgEls[emotion];
				if (img) {
					if (!this._imgLoaded[emotion]) {
						img.src = PET_ASSET_BASE + emotion + ".webp";
						this._imgLoaded[emotion] = true;
					}
		...
		let petConfig = { enabled: true, size: "medium", position: "bottom-right", posOverride: null, opacity: 1 };
		let petLastStats = null;
		let petBubbleT = (k) => k;

		function petPersist(patch) {
			return fetch("/personal-center/pet", {
				method: "POST",
				headers: { "content-type": "application/json" },
		...
		function petHide() {
			...
			petPersist({ enabled: false }).then(() => petApplyConfig({ enabled: false })).catch(() => {});
		}

		/** 右键菜单改尺寸(持久化 + 应用)。 */
		function petResize(size) {
			petPersist({ size }).then(() => petApplyConfig({ size })).catch(() => {});
		}
		...
		function petApplyConfig(cfg) {
			...
			if (cfg.size && cfg.size !== prev.size) petWidget.setSize(cfg.size);
			if (cfg.position && cfg.position !== prev.position) petWidget.setPosition(cfg.position);
			if (cfg.opacity !== undefined && cfg.opacity !== prev.opacity) petWidget.setOpacity(cfg.opacity);
		}

		/**
		 * 桌面宠物配置面板:启用/尺寸/位置/不透明度 + 今日情绪 + 阈值说明。
		...
		function PetPanel({ t, petCtl }) {
			...
			react.useEffect(() => {
				...
				petCtl.getConfig().then((c) => {
					...
					// 宿主未重启/接口不可用:回退默认值,面板仍可打开并显示重启提示
					if (alive) setCfg({ enabled: true, size: "medium", position: "bottom-right", posOverride: null, opacity: 1 });
				});
				const sync = () => {
			...
			const update = (patch) => {
				setCfg((c) => ({ ...c, ...patch }));
				setSaving(true);
				petCtl.applyConfig(patch).then(() => setSaving(false)).catch(() => setSaving(false));
			};
			...
			const emoKey = PET_EMOTION_LABEL_KEYS[emotion] || "petEmotionHappy";
			const emoAsset = PET_ASSET_BASE + (emotion || "happy") + ".webp";
			const th = PET_THRESHOLDS;
			...
			const pill = (id, label, active, onPick) => react.createElement("button", {
			...
			const thresholdRow = (label, value) => react.createElement("div", { className: "dsh-pc-pet-threshold" },
				react.createElement("span", null, label),
				react.createElement("b", null, value)
			);
			const posLabel = { "bottom-right": t("petPosBr"), "bottom-left": t("petPosBl"), "top-right": t("petPosTr"), "top-left": t("petPosTl") };

			return react.createElement("div", { className: "dsh-pc-panel" },
				react.createElement("div", { className: "dsh-pc-pet-card" },
					react.createElement("img", { className: "dsh-pc-pet-preview-img", src: emoAsset, alt: "" }),
					react.createElement("div", { className: "dsh-pc-pet-info" },
						react.createElement("div", { className: "dsh-pc-activity-head" },
							react.createElement("h3", { className: "dsh-pc-group dsh-pc-activity-title" }, t("petTitle")),
							react.createElement("span", { className: "dsh-pc-pet-badge" }, cfg.enabled ? t("petStatusOn") : t("petStatusOff"))
						),
						react.createElement("p", { className: "dsh-pc-mock" },
							interp("petTodayStats", { tokens: statTokens, tools: statTools, rate: statRate }))
					)
				),
				react.createElement("div", { className: "dsh-pc-pet-opts" },
					react.createElement("div", { className: "dsh-pc-pet-opt" },
						react.createElement("label", { className: "dsh-pc-pet-toggle" },
							react.createElement("input", { type: "checkbox", checked: cfg.enabled, onChange: (e) => update({ enabled: e.target.checked }) }),
							t("petEnable")
						),
						saving ? react.createElement("span", { className: "dsh-pc-pet-badge" }, t("petSaved")) : null
					),
					react.createElement("div", { className: "dsh-pc-pet-opt" },
						react.createElement("span", { className: "dsh-pc-pet-opt-label" }, t("petSize")),
						react.createElement("div", { className: "dsh-pc-pill-tabs" },
							[["small", "S"], ["medium", "M"], ["large", "L"]].map(([id, label]) => pill(id, label, cfg.size === id, () => update({ size: id })))
						)
					),
					react.createElement("div", { className: "dsh-pc-pet-opt" },
						react.createElement("span", { className: "dsh-pc-pet-opt-label" }, t("petPosition")),
						react.createElement("div", { className: "dsh-pc-pill-tabs" },
							["bottom-right", "bottom-left", "top-right", "top-left"].map((id) => pill(id, posLabel[id], cfg.position === id, () => update({ position: id })))
						)
					),
					react.createElement("div", { className: "dsh-pc-pet-opt" },
						react.createElement("label", { className: "dsh-pc-pet-toggle" },
							t("petOpacity"),
							react.createElement("input", { className: "dsh-pc-pet-range", type: "range", min: "0.3", max: "1", step: "0.05", value: String(cfg.opacity), onChange: (e) => update({ opacity: Number(e.target.value) }) }),
							Math.round(cfg.opacity * 100) + "%"
						)
					)
				),
				react.createElement("div", { className: "dsh-pc-pet-emotion" },
					react.createElement("img", { className: "dsh-pc-pet-emotion-img", src: emoAsset, alt: "" }),
					react.createElement("div", { className: "dsh-pc-pet-info" },
						react.createElement("div", { className: "dsh-pc-pet-emotion-name" }, t("petToday") + " · " + t(emoKey)),
						react.createElement("p", { className: "dsh-pc-pet-emotion-sub" },
							interp("petTodayStats", { tokens: statTokens, tools: statTools, rate: statRate }))
					)
				),
				react.createElement("p", { className: "dsh-pc-pet-hint" }, t("petThresholdsTitle")),
				react.createElement("div", { className: "dsh-pc-pet-thresholds" },
					thresholdRow(interp("petThresholdHappy", { v: Math.round(th.happy_cache_rate * 100) + "%" }), "≥" + Math.round(th.happy_cache_rate * 100) + "%"),
					thresholdRow(interp("petThresholdBusy", { v: th.busy_tool_calls }), "≥" + th.busy_tool_calls + " 次"),
					thresholdRow(interp("petThresholdExhausted", { v: petFmtNum(th.exhausted_tokens) }), "≥" + petFmtNum(th.exhausted_tokens)),
					thresholdRow(interp("petThresholdMoneyPain", { v: th.money_pain_cost }), "≥¥" + th.money_pain_cost),
					thresholdRow(interp("petThresholdDozing", { v: th.dozing_idle_minutes }), "≥" + th.dozing_idle_minutes + " 分钟")
				),
				react.createElement("p", { className: "dsh-pc-pet-hint" }, t("petNoPriceHint")),
				react.createElement("p", { className: "dsh-pc-pet-hint" }, t("petRestartHint"))
			);
		}
		//#endregion
		...
		function apply(ctx) {
			...
			const petCtl = {
				applyConfig: (patch) => petPersist(patch).then(() => petApplyConfig(patch)),
