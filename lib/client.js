	...
	factory: (require) => {
		...
		class PetStatusPanel {
			...
			_announce(fail, run, done, sub) {
				...
				// 情绪映射(优先级 失败 > 等你响应 > 思考中 > 忙碌 > 完成庆祝 > 打盹):
				// 失败 exhausted / waiting(等你回复)/ thinking(思考中)/ busy / celebrate / dozing
				const emo = fail > 0 ? "exhausted"
					: (sub.waiting > 0) ? "waiting"
					: (sub.thinking > 0) ? "thinking"
					: (run > 0) ? "busy"
					: (done > 0) ? "celebrate"
					: "dozing";
				this.pet.setEmotion(emo);
			}
		...
		function petMount() {
			...
			// 常驻会话情绪同步:thinking/waiting/celebrate 不依赖面板打开
		...
		function petStatusEmotionOf() {
			...
			for (const id of rows) {
				...
				} else if (entry.completed) done += 1;
				else if (s && s.getSnapshot && s.getSnapshot().lastAgentError) failed += 1;
			}
			if (failed > 0) return "exhausted";
			if (waiting > 0) return "waiting";
			if (thinking > 0) return "thinking";
			if (running > 0) return "busy";
			if (done > 0) return "celebrate";
			return "dozing"; // 空闲:打盹(stats 轮询会按真实数据再覆盖)
		}
		...
		function petStatusEmotionSync() {
			...
			const apply = () => {
				...
				if (emo && petWidget) petWidget.setEmotion(emo);
			};
			apply();
