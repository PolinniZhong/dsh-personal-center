	...
	factory: (require) => {
		...
		class PetWidget {
			...
			_openMenu(x, y) {
				...
				// 紧贴宠物下方:向上移 32px(相对 +12),菜单顶部与宠物容器底部重叠 20px,视觉紧贴鲸鱼身体
				this._menuEl.style.top = Math.max(0, Math.min(rect.bottom - 20, window.innerHeight - 60)) + "px";
			}
			...
			_bindEvents() {
				this._onClick = (e) => {
					...
					this.playAction();
					// 会话状态开启时隐藏数据气泡(Token 用量),退出后恢复显示。
					// 打开面板:不弹数据气泡(状态由面板列表展示);关闭面板:弹数据气泡。
		...
		class PetStatusPanel {
			...
			position() {
				...
				// 面板在宠物上方,留 64px 间距(向上移 40px,远离宠物);上方放不下则翻到下方
				if (rect.top - 64 - h >= 8) {
					this.el.style.top = rect.top - 64 - h + "px";
					this.el.style.bottom = "auto";
				} else {
					this.el.style.top = "auto";
					this.el.style.bottom = Math.max(8, window.innerHeight - rect.bottom + 24) + "px";
				}
			}
