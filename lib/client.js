 ...
 *
 * v0.2(UI 预览版):设置里新增「个人」分区,内含 tab 栏:
 *   - 「个人资料」:累计 Token / 最长聊天时长 / 常用工具 等统计卡片。
 *     当前为【示例数据】,统计 RPC 接入后显示真实用量(见 docs/PLAN.md M2/M3);
 *   - 「个性化」:自定义指令多行文本框(原「个性化」功能迁入,数据不丢)。
 *
 * 预留:未来 Omi 等插件可停靠「语音」tab(声明 personal-center.tab 槽位,本期不做)。
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			/* 公共 */
			".dsh-pc-section{display:flex;flex-direction:column;gap:12px;padding:0 4px 20px}" +
			/* tab 栏 */
			".dsh-pc-tabs{display:flex;gap:4px;border-bottom:1px solid var(--dsw-alias-border-l2);margin-bottom:16px}" +
			".dsh-pc-tab{cursor:pointer;border:none;background:0 0;color:var(--dsw-alias-label-secondary);font-family:inherit;font-size:14px;line-height:22px;padding:8px 16px;border-radius:8px 8px 0 0}" +
			".dsh-pc-tab:hover:not(.dsh-pc-tab-active){background:var(--dsw-alias-interactive-bg-hover)}" +
			".dsh-pc-tab-active{color:var(--dsw-alias-label-primary);font-weight:500;box-shadow:inset 0 -2px 0 var(--dsw-static-neutral-bluish-400)}" +
			/* 统计卡片 */
			...
			".dsh-pc-card{background:var(--dsw-alias-bg-module-platform);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;padding:14px 16px;display:flex;flex-direction:column;gap:4px}" +
			".dsh-pc-card-label{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}" +
			".dsh-pc-card-value{color:var(--dsw-alias-label-primary);font-size:22px;font-weight:600;line-height:30px;font-variant-numeric:tabular-nums}" +
			".dsh-pc-card-sub{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}" +
			/* 常用工具列表 */
		...
		function StatCard(label, value, sub) {
			return react.createElement("div", { className: "dsh-pc-card" },
				react.createElement("div", { className: "dsh-pc-card-label" }, label),
				react.createElement("div", { className: "dsh-pc-card-value" }, value),
				sub !== void 0 && react.createElement("div", { className: "dsh-pc-card-sub" }, sub)
			);
		}
		...
		//#region personal center section(tab 容器)
		/**
		 * tab 按钮。
		 * @param {string} id
		 * @param {string} label
		...
		function TabButton(id, label, active, setTab) {
			return react.createElement("button", {
				className: "dsh-pc-tab" + (active === id ? " dsh-pc-tab-active" : ""),
				onClick: () => setTab(id)
			}, label);
		}
		...
		function PersonalCenterSection({ t, scope }) {
			...
			return react.createElement("div", { className: "dsh-pc-section" },
				react.createElement("div", { className: "dsh-pc-tabs" },
				...
				tab === "profile"
					...
					: react.createElement(CustomInstructionsSection, { t, scope })
			);
		}
		//#endregion
