...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-pc-pet-switch-thumb{position:absolute;top:2px;left:2px;width:16px;height:16px;border-radius:50%;background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.28);transition:transform .2s ease}" +
		...
		function PetPanel({ t, petCtl }) {
			...
			react.useEffect(() => {
				...
				petCtl.getConfig().then((c) => {
					...
					if (!alive) return;
					setServiceDown(true);
					setCfg({ enabled: true, size: "small", position: "bottom-right", posOverride: null, opacity: 1 });
				});
				const sync = () => {
