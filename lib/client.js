...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-inst-ws-item{border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:10px 12px}" +
			".dsh-inst-ws-item[data-current=true]{border-color:var(--dsw-alias-state-business-primary)}" +
			...
			/* 工作区选择器:点击即下拉(自定义,替代 datalist——datalist 在设置面板不触发下拉) */
			".dsh-inst-ws-picker{position:relative;display:flex;align-items:center;gap:4px;margin-top:12px;min-width:0}" +
			".dsh-inst-ws-picker input{box-sizing:border-box;flex:1;min-width:0;background:transparent;color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;height:28px;padding:0 10px;font-size:12px;font-family:inherit;outline:none}" +
			".dsh-inst-ws-picker input:focus{border-color:var(--dsw-static-neutral-bluish-400)}" +
			".dsh-inst-ws-caret{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-secondary);cursor:pointer;border-radius:8px;height:28px;width:30px;flex:none;display:inline-flex;align-items:center;justify-content:center}" +
			".dsh-inst-ws-caret:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}" +
			".dsh-inst-ws-menu{position:absolute;top:calc(100% + 4px);left:0;right:30px;z-index:50;background:var(--dsw-alias-bg-module-platform);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;max-height:200px;overflow-y:auto;box-shadow:var(--dsw-shadow-lv3);padding:4px}" +
				...
				Promise.all([
					...
					if (ins && ins.ok !== false) {
						if (ins.workspaces && typeof ins.workspaces === "object") {
							setWsMap(ins.workspaces);
							const drafts = {};
							for (const k of Object.keys(ins.workspaces)) drafts[k] = ins.workspaces[k];
							setWsDrafts(drafts);
						}
								...
								wsKeys.map((key) => {
									...
									return react.createElement("div", { key, className: "dsh-inst-ws-item", "data-current": isCurrent },
										react.createElement("div", { className: "dsh-inst-ws-head" },
											...
											react.createElement("span", { className: "dsh-inst-ws-tag", "data-current": isCurrent }, isCurrent ? t("ciWsCurrent") : t("ciWsFallback")),
					...
					react.createElement("div", { className: "dsh-inst-ws-picker" },
						...
						wsOpen && react.createElement("div", { className: "dsh-inst-ws-menu" },
							discovered.map((p) => react.createElement("button", {
								...
								onMouseDown: (e) => e.preventDefault(), // 避免 blur 先关菜单
