...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			/* 矢量宠物(圆滚滚小黑/小蓝:纯 DOM,非 WebP,鼠标跟随四层) */
			".dsh-pet-vector-root{position:absolute;inset:0}" +
			".dsh-pet-vector-shift,.dsh-pet-vector-deform{position:absolute;inset:0;will-change:transform}" +
			".dsh-pet-vector-body{position:absolute;inset:0;border-radius:50%;background:radial-gradient(circle at var(--lx,50%) var(--ly,45%),var(--c0) 0%,var(--c1) 35%,var(--c2) 70%,var(--c3) 92%,var(--c4) 100%);filter:drop-shadow(var(--glow));animation:dsh-pet-vector-breathe 3.2s ease-in-out infinite}" +
			".dsh-pet-vector-body.following{animation-play-state:paused}" +
			".dsh-pet-vector-eye{position:absolute;top:36%;width:10.9%;height:20%}" +
			".dsh-pet-vector-eye.left{left:30%}" +
			".dsh-pet-vector-eye.right{right:30%}" +
			".dsh-pet-vector-eye-inner{position:absolute;inset:0;background:rgba(255,255,255,.95);border-radius:999px;filter:drop-shadow(var(--eye-glow));will-change:transform}" +
			"@keyframes dsh-pet-vector-breathe{0%,100%{transform:scale(1)}50%{transform:scale(1.03)}}" +
			".dsh-pet-vector.blue{--c0:#1E6FFF;--c1:#3B82F6;--c2:#7DB4F8;--c3:#C8DFFB;--c4:rgba(200,223,251,0);--glow:0 0 12px rgba(30,111,255,.25);--eye-glow:0 0 4px rgba(30,111,255,.20)}" +
			".dsh-pet-vector.black{--c0:#0A0A0A;--c1:#2A2A2A;--c2:#666666;--c3:#C8C8C8;--c4:rgba(200,200,200,0);--glow:0 0 12px rgba(40,40,40,.30);--eye-glow:0 0 4px rgba(0,0,0,.10)}" +
			".dsh-pet-container.dragging .dsh-pet-vector-body{animation:none!important}" +
			...
			".dsh-pc-pet-vector-preview{position:relative;width:56px;height:56px;flex:none}" +
		...
		const dictionaries = {
			zh: {
				...
				"petNameBlack": "圆滚滚小黑鲸",
				"petNameBlue": "圆滚滚小蓝鲸",
				"petNameBlackVector": "圆滚滚小黑",
				"petNameBlueVector": "圆滚滚小蓝",
				"petPosition": "位置",
				"petDev": "开发中",
				"petSize": "尺寸",
				"petOpacity": "不透明度",
			...
			en: {
				...
				"petNameBlackVector": "Black Whale (Vector)",
				"petNameBlueVector": "Blue Whale (Vector)",
		...
		const PET_SKINS = ["black-whale", "blue-whale", "black-vector", "blue-vector"];
		/** 矢量皮肤(纯 DOM 渲染,无 WebP 素材):圆滚滚小黑(black-vector)/ 圆滚滚小蓝(blue-vector)。 */
		const PET_VECTOR_SKINS = ["black-vector", "blue-vector"];
		/** 位图皮肤(有 WebP 素材;面板随机表情轮播只在此范围内)。 */
		const PET_BITMAP_SKINS = ["black-whale", "blue-whale"];
		/**
		...
		/**
		 * 矢量版宠物浮层(圆滚滚小黑 / 圆滚滚小蓝):纯 DOM 渲染,非 WebP。
		 * 核心 = 鼠标跟随四层(①眼睛朝向 ②头位移 ③上下"驼"形变 ④渐变光源)+ 转向透视
		 * (转向侧眼睛变小 + 两眼间距压缩)+ 交互(靠近脸眨眼 / 点眼睛眯成线 / 点身体挠痒 /
		 * 随机头放大)。复用位图版外壳:拖拽定位、数据气泡、右键隐藏、会话状态按钮。
		 * 无情绪动画(矢量版不做业务情绪 WebP 动画),情绪接口为 no-op。
		 */
		const VEC = {
			HOT_ZONE: 280, EYE_SHIFT: 15, BODY_SHIFT: 12, LIGHT_SHIFT: 0.15,
			K_EYE: 0.18, K_BODY: 0.15, SNAP: 0.01,
			STRETCH: 0.05, SIZE_P: 0.18, SPACING_SHIFT: 4,
			BLINK_RADIUS: 45, BLINK_DUR: 220,
			POP_DUR: 380, POP_SCALE: 1.07,
			TICKLE_DUR: 650, TICKLE_SHAKE: 1.8, TICKLE_SQUINT: 0.45,
			SQUINT_LINE_DUR: 280
		};
		function vecClamp(v, lo, hi) { return v < lo ? lo : (v > hi ? hi : v); }
		function vecRand(min, max) { return min + Math.random() * (max - min); }
		function vecLerp(cur, target, k) { if (Math.abs(target - cur) < VEC.SNAP) return target; return cur + (target - cur) * k; }
		function vecColorClass(skin) { return skin === "blue-vector" ? "blue" : "black"; }

		class VectorPetWidget {
			constructor(o = {}) {
				this.options = {
					container: document.body,
					position: "bottom-right",
				...
				this._cur = { eyeX: 0, eyeY: 0, shiftX: 0, shiftY: 0, sx: 1, sy: 1, lx: 50, ly: 45, sizeL: 1, sizeR: 1, spacingIn: 0 };
				this._following = false;
				this._mouseX = null;
				this._mouseY = null;
				this._wasNearEye = false;
				this._blinkStart = -1e9;
				this._tickleStart = -1e9;
				this._squintStart = -1e9;
				this._popStart = -1e9;
				this._nextPopAt = performance.now() + vecRand(4200, 9500);
				this._lastStats = {};
				this._hasStats = false;
				this._bubbleTimer = null;
				this._dragState = null;
			...
			setEmotion() { /* 矢量版无情绪动画:no-op */ }
			setSessionEmotion() { /* no-op */ }
			playAction() { /* no-op */ }
			playOnce() { /* no-op */ }
			getCurrentEmotion() { return null; }
			...
			/** 切换矢量皮肤:只换配色 class(结构一致,无需重载素材)。 */
			setSkin(skin) {
				if (PET_VECTOR_SKINS.indexOf(skin) < 0) return;
				this._containerEl.classList.remove("blue", "black");
				this._containerEl.classList.add(vecColorClass(skin));
				this.options.skin = skin;
			}
			show() { this._containerEl.classList.remove("hidden"); }
			hide() { this._containerEl.classList.add("hidden"); }
			...
			_buildDOM() {
				this._containerEl = document.createElement("div");
				this._containerEl.className = "dsh-pet-container dsh-pet-vector " + vecColorClass(this.options.skin) + " size-" + this.options.size;
				this._containerEl.style.opacity = this.options.opacity;

				this._wrapperEl = document.createElement("div");
				this._wrapperEl.className = "dsh-pet-wrapper";
				this._rootEl = document.createElement("div");
				this._rootEl.className = "dsh-pet-vector-root";
				this._shiftEl = document.createElement("div");
				this._shiftEl.className = "dsh-pet-vector-shift";
				this._deformEl = document.createElement("div");
				this._deformEl.className = "dsh-pet-vector-deform";
				this._bodyEl = document.createElement("div");
				this._bodyEl.className = "dsh-pet-vector-body";

				const eye = (side) => {
					const host = document.createElement("span");
					host.className = "dsh-pet-vector-eye " + side;
					const inner = document.createElement("span");
					inner.className = "dsh-pet-vector-eye-inner";
					host.appendChild(inner);
					return host;
				};
				this._eyeLHost = eye("left");
				this._eyeRHost = eye("right");
				this._eyeL = this._eyeLHost.querySelector(".dsh-pet-vector-eye-inner");
				this._eyeR = this._eyeRHost.querySelector(".dsh-pet-vector-eye-inner");
				this._bodyEl.appendChild(this._eyeLHost);
				this._bodyEl.appendChild(this._eyeRHost);

				this._deformEl.appendChild(this._bodyEl);
				this._shiftEl.appendChild(this._deformEl);
				this._rootEl.appendChild(this._shiftEl);
				this._wrapperEl.appendChild(this._rootEl);
			...
			_buildMenu() {
				...
				const hide = document.createElement("div");
				hide.className = "dsh-pet-menu-item";
				hide.textContent = this.options.t("petMenuHide");
				hide.addEventListener("click", () => {
					this._closeMenu();
					if (this.options.onHide) this.options.onHide();
				});
			...
			_bindEvents() {
				...
				this._onStatusBtnClick = (e) => {
					...
					if (this.options.onStatusToggle) {
						const wasOpen = !!petStatusPanel;
						this.options.onStatusToggle();
						if (wasOpen) this.showBubble(this._pickBubble());
				...
				// 点眼睛 → 眯成线(阻止冒泡,不触发身体挠痒/拖拽)
				this._onEyePointerDown = (e) => {
					e.stopPropagation();
					this._suppressClick = true; // 点眼睛只眯线,不触发身体挠痒(click 冒泡)
					this.squint();
				};
				this._eyeLHost.addEventListener("pointerdown", this._onEyePointerDown);
				this._eyeRHost.addEventListener("pointerdown", this._onEyePointerDown);

				// 拖拽
				this._onPointerDown = (e) => this._startDrag(e);
				this._containerEl.addEventListener("pointerdown", this._onPointerDown);

				// 鼠标跟随:全局 mousemove + 离开窗口清除
				this._onMouseMove = (e) => { this._mouseX = e.clientX; this._mouseY = e.clientY; };
				window.addEventListener("mousemove", this._onMouseMove);
				this._onMouseLeave = () => { this._mouseX = null; this._mouseY = null; };
			...
			_unbindEvents() {
				this._containerEl.removeEventListener("click", this._onClick);
				this._statusBtnEl.removeEventListener("click", this._onStatusBtnClick);
				this._containerEl.removeEventListener("contextmenu", this._onContextMenu);
				if (this._onDocClick) document.removeEventListener("click", this._onDocClick);
				this._eyeLHost.removeEventListener("pointerdown", this._onEyePointerDown);
				this._eyeRHost.removeEventListener("pointerdown", this._onEyePointerDown);
			...
			/* ── 拖拽(复用位图版逻辑,拖拽态停呼吸+放大旋转由 CSS 处理) ── */

			_startDrag(e) {
				e.preventDefault();
				this._dragState = { startX: e.clientX, startY: e.clientY, offsetX: e.clientX - this._containerEl.getBoundingClientRect().left, offsetY: e.clientY - this._containerEl.getBoundingClientRect().top, moved: false };
				this._containerEl.classList.add("dragging");
				this._onPointerMove = (ev) => this._onDrag(ev);
				this._onPointerUp = () => this._endDrag();
				this._onPointerCancel = () => this._endDrag();
			...
			_onDrag(e) {
				...
				const dx = e.clientX - this._dragState.startX;
				const dy = e.clientY - this._dragState.startY;
				if (Math.abs(dx) > 4 || Math.abs(dy) > 4) { this._dragState.moved = true; this._suppressClick = true; }
				const w = this._containerEl.offsetWidth;
				const h = this._containerEl.offsetHeight;
				const x = Math.max(0, Math.min(window.innerWidth - w, e.clientX - this._dragState.offsetX));
				const y = Math.max(0, Math.min(window.innerHeight - h, e.clientY - this._dragState.offsetY));
			...
			blink() {
				...
				if (now - this._blinkStart > VEC.BLINK_DUR + 160) this._blinkStart = now;
			}
			tickle() {
				this._tickleStart = performance.now();
			}
			squint() {
				this._squintStart = performance.now();
			}
			...
			_step() {
				...
				let t = 0, angle = 0, dist = 1e9;
				if (this._mouseX !== null) {
					const dx = this._mouseX - cx;
					const dy = this._mouseY - cy;
					dist = Math.sqrt(dx * dx + dy * dy);
					angle = Math.atan2(dy, dx);
					t = vecClamp(1 - dist / VEC.HOT_ZONE, 0, 1);
				}
				const cos = Math.cos(angle), sin = Math.sin(angle);
				const c = this._cur;

				const eyeTx = t > 0 ? cos * VEC.EYE_SHIFT : 0;
				const eyeTy = t > 0 ? sin * VEC.EYE_SHIFT : 0;
				const shiftTx = cos * VEC.BODY_SHIFT * t;
				const shiftTy = sin * VEC.BODY_SHIFT * t;
				const sxT = 1 - VEC.STRETCH * Math.abs(sin) * t;
				const syT = 1 + VEC.STRETCH * Math.abs(sin) * t;
				const lxT = 50 + cos * VEC.LIGHT_SHIFT * 100 * t;
				const lyT = 45 + sin * VEC.LIGHT_SHIFT * 100 * t;
				const sizeLT = 1 - VEC.SIZE_P * Math.max(0, -cos) * t;
				const sizeRT = 1 - VEC.SIZE_P * Math.max(0, cos) * t;
				const spacingT = VEC.SPACING_SHIFT * Math.abs(cos) * t;

				c.eyeX = vecLerp(c.eyeX, eyeTx, VEC.K_EYE);
				c.eyeY = vecLerp(c.eyeY, eyeTy, VEC.K_EYE);
				c.shiftX = vecLerp(c.shiftX, shiftTx, VEC.K_BODY);
				c.shiftY = vecLerp(c.shiftY, shiftTy, VEC.K_BODY);
				c.sx = vecLerp(c.sx, sxT, VEC.K_BODY);
				c.sy = vecLerp(c.sy, syT, VEC.K_BODY);
				c.lx = vecLerp(c.lx, lxT, VEC.K_BODY);
				c.ly = vecLerp(c.ly, lyT, VEC.K_BODY);
				c.sizeL = vecLerp(c.sizeL, sizeLT, VEC.K_EYE);
				c.sizeR = vecLerp(c.sizeR, sizeRT, VEC.K_EYE);
				c.spacingIn = vecLerp(c.spacingIn, spacingT, VEC.K_EYE);

				// 靠近脸(进入上升沿)→ 眨眼
				const nearEye = dist < VEC.BLINK_RADIUS;
				if (nearEye && !this._wasNearEye) this.blink();
				this._wasNearEye = nearEye;

				// 跟随中随机头放大
				if (t > 0 && now >= this._nextPopAt) {
					this._popStart = now;
					this._nextPopAt = now + vecRand(4200, 9500);
				}

				let blinkY = 1;
				const bp = (now - this._blinkStart) / VEC.BLINK_DUR;
				if (bp < 1) blinkY = 1 - 0.92 * Math.max(0, Math.sin(2 * Math.PI * bp));

				let shakeX = 0, shakeY = 0, squintY = 1;
				const tp = (now - this._tickleStart) / VEC.TICKLE_DUR;
				if (tp < 1) {
					const env = Math.sin(Math.PI * tp);
					shakeX = Math.sin(2 * Math.PI * 9 * tp) * VEC.TICKLE_SHAKE * env;
					shakeY = Math.sin(2 * Math.PI * 7 * tp) * VEC.TICKLE_SHAKE * 0.5 * env;
					squintY = 1 - VEC.TICKLE_SQUINT * (0.5 - 0.5 * Math.cos(2 * Math.PI * 3 * tp)) * env;
				}

				let squintLineY = 1;
				const sp = (now - this._squintStart) / VEC.SQUINT_LINE_DUR;
				if (sp < 1) squintLineY = 1 - 0.95 * Math.sin(Math.PI * sp);

				let pop = 1;
				const pp = (now - this._popStart) / VEC.POP_DUR;
				if (pp < 1) pop = 1 + (VEC.POP_SCALE - 1) * Math.sin(Math.PI * pp);

				this._shiftEl.style.transform = "translate(" + (c.shiftX + shakeX) + "px, " + (c.shiftY + shakeY) + "px)";
				this._deformEl.style.transform = "scale(" + (c.sx * pop) + ", " + (c.sy * pop) + ")";
				this._bodyEl.style.setProperty("--lx", c.lx + "%");
				this._bodyEl.style.setProperty("--ly", c.ly + "%");
				const eyeSY = blinkY * squintY * squintLineY;
				this._eyeL.style.transform = "translate(" + (c.eyeX + c.spacingIn) + "px, " + c.eyeY + "px) scale(" + c.sizeL + ", " + (c.sizeL * eyeSY) + ")";
				this._eyeR.style.transform = "translate(" + (c.eyeX - c.spacingIn) + "px, " + c.eyeY + "px) scale(" + c.sizeR + ", " + (c.sizeR * eyeSY) + ")";

				const isFollowing = t > 0;
				if (isFollowing !== this._following) {
					this._following = isFollowing;
					this._bodyEl.classList.toggle("following", isFollowing);
				}
			}
		}
		...
		/** 按皮肤类型实例化浮层宠物:矢量皮肤用 VectorPetWidget(纯 DOM),其余用 PetWidget(WebP)。 */
		function petBuildWidget() {
			const SkinWidget = PET_VECTOR_SKINS.indexOf(petConfig.enabled) >= 0 ? VectorPetWidget : PetWidget;
			petWidget = new SkinWidget({
				container: document.body,
				position: "bottom-right",
		...
		function petApplyConfig(cfg) {
			...
			// 启用的宠物变化(互斥切换)或选中皮肤变化 → 切换皮肤
			const target = cfg.enabled !== undefined ? petConfig.enabled : cfg.skin;
			if (target && target !== petWidget.options.skin) {
				// 位图↔矢量类型不同:实例类不同,销毁重建(轮询/会话状态订阅不重开,只换实例)
				const isVector = PET_VECTOR_SKINS.indexOf(target) >= 0;
				const wasVector = PET_VECTOR_SKINS.indexOf(petWidget.options.skin) >= 0;
				if (isVector !== wasVector) {
					petWidget.destroy();
					petWidget = null;
					petBuildWidget();
					petFetchStats(); // 立即喂一次 stats(气泡数据)
					petApplySessionStatus();
				} else {
					petWidget.setSkin(target);
				}
			}
		...
		function PetPanel({ t, petCtl }) {
			...
			react.useEffect(() => {
				...
				const playNext = () => {
					if (!alive) return;
					const id = PET_BITMAP_SKINS[turn % PET_BITMAP_SKINS.length];
					turn += 1;
			...
			// 皮肤显示名:位图(圆滚滚小黑鲸/小蓝鲸)+ 矢量(圆滚滚小黑/小蓝)
			const petTitle = (id) => id === "black-whale" ? t("petNameBlack")
				: id === "blue-whale" ? t("petNameBlue")
				: id === "blue-vector" ? t("petNameBlueVector")
				: t("petNameBlackVector");
			// 矢量皮肤预览:纯 DOM(圆形身体 + 胶囊眼 + 呼吸),非 WebP
			const vectorPreview = (id) => react.createElement("div", { className: "dsh-pc-pet-vector-preview dsh-pet-vector " + vecColorClass(id) },
				react.createElement("div", { className: "dsh-pet-vector-body" },
					react.createElement("span", { className: "dsh-pet-vector-eye left" }, react.createElement("span", { className: "dsh-pet-vector-eye-inner" })),
					react.createElement("span", { className: "dsh-pet-vector-eye right" }, react.createElement("span", { className: "dsh-pet-vector-eye-inner" }))
				)
			);
			const petCard = (id) => {
				const isVector = PET_VECTOR_SKINS.indexOf(id) >= 0;
				...
				return react.createElement("div", {
					...
					react.createElement("div", { className: "dsh-pc-pet-info" },
						react.createElement("div", { className: "dsh-pc-pet-title-row" },
							react.createElement("span", { className: "dsh-pc-pet-title" }, petTitle(id)),
