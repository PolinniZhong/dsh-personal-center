...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			/* 数据气泡:毛玻璃(半透明 + 背景模糊,参考 Codex/苹果;深浅主题各一套) */
			".dsh-pet-bubble{position:absolute;bottom:calc(100% + 12px);left:50%;transform:translateX(-50%) translateY(8px);background:rgba(255,255,255,.45);-webkit-backdrop-filter:blur(14px) saturate(1.5);backdrop-filter:blur(14px) saturate(1.5);color:var(--dsw-alias-label-primary);border:1px solid rgba(255,255,255,.55);padding:10px 16px;border-radius:16px;font-size:13px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .25s ease,transform .25s ease;max-width:min(280px,60vw)}" +
			".dsh-pet-bubble.visible{opacity:1;transform:translateX(-50%) translateY(0)}" +
			".dsh-pet-bubble::after{content:\"\";position:absolute;top:100%;left:50%;transform:translateX(-50%);border:6px solid transparent;border-top-color:rgba(255,255,255,.45)}" +
			...
			/* 宠物名字(随情绪变,显示在宠物下方)+ 悬停提示「点击查看会话状态」 */
			".dsh-pet-name{position:absolute;top:calc(100% + 3px);left:0;right:0;text-align:center;font-size:10px;line-height:14px;color:var(--dsw-alias-label-tertiary);white-space:nowrap;pointer-events:none}" +
			".dsh-pet-hint{position:absolute;top:calc(100% + 19px);left:0;right:0;text-align:center;font-size:10px;line-height:14px;color:var(--dsw-alias-label-secondary);white-space:nowrap;pointer-events:none;display:none}" +
			".dsh-pet-container:hover .dsh-pet-hint{display:block}" +
			...
			".dsh-pet-menu{position:fixed;z-index:2147483646;background:rgba(255,255,255,.45);-webkit-backdrop-filter:blur(14px) saturate(1.5);backdrop-filter:blur(14px) saturate(1.5);border:1px solid rgba(255,255,255,.55);border-radius:16px;padding:2px;min-width:88px;box-shadow:0 8px 24px rgba(0,0,0,.08);display:none}" +
			...
			".dsh-pet-menu-item{display:flex;align-items:center;justify-content:center;width:100%;padding:6px 10px;border-radius:10px;font-size:12px;line-height:18px;color:var(--dsw-alias-label-primary);cursor:pointer;white-space:nowrap}" +
			...
			/* 宠物状态概览:毛玻璃浮层(复用气泡配方),锚定宠物上方/下方,懒挂载 */
			".dsh-pet-status{position:fixed;z-index:2147483001;width:320px;max-width:calc(100vw - 24px);background:rgba(255,255,255,.45);-webkit-backdrop-filter:blur(14px) saturate(1.5);backdrop-filter:blur(14px) saturate(1.5);color:var(--dsw-alias-label-primary);border:1px solid rgba(255,255,255,.55);border-radius:16px;box-shadow:0 8px 24px rgba(0,0,0,.08);overflow:hidden;display:none}" +
			...
			".dsh-pet-status-head .dsh-pet-status-hint{margin-left:auto;font-size:10px;font-weight:400;color:var(--dsw-alias-label-tertiary)}" +
			".dsh-pet-status-sum{display:flex;gap:18px;padding:12px 16px 8px;font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary)}" +
			".dsh-pet-status-sum b{display:block;font-size:20px;font-weight:600;line-height:26px;font-variant-numeric:tabular-nums}" +
			...
			".dsh-pet-status-list{max-height:252px;overflow-y:auto;padding:6px 10px 12px}" +
			...
			".dsh-pet-status-name{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-primary)}" +
		...
		const dictionaries = {
			zh: {
				...
				"petStatusHint": "点外部或 ESC 关闭",
				"petStatusHintLine": "点击查看会话状态 ▾",
			...
			en: {
				...
				"petStatusHint": "Click outside or press ESC to close",
				"petStatusHintLine": "Click to view session status ▾",
		...
		class PetWidget {
			...
			_buildDOM() {
				...
				// 宠物名字(随情绪变,显示在宠物下方)+ 悬停提示(入口可发现,参考状态概览 Demo)
				this._nameEl = document.createElement("div");
				this._nameEl.className = "dsh-pet-name";
				this._containerEl.appendChild(this._nameEl);
				this._hintEl = document.createElement("div");
				this._hintEl.className = "dsh-pet-hint";
				this._hintEl.textContent = this.options.t("petStatusHintLine");
				this._containerEl.appendChild(this._hintEl);
				this._updateName();
			...
			_openMenu(x, y) {
				...
				// 下移 32px:避免菜单压在宠物身体上(右键点在宠物身上)
				this._menuEl.style.top = Math.max(0, Math.min(y + 32, window.innerHeight - 80)) + "px";
			...
			_bindEvents() {
				this._onClick = (e) => {
					...
					this.playAction();
					// 点击必有气泡反馈:先弹随机数据气泡,再 toggle 状态面板;
					// 面板打开时状态气泡(失败/进行中/完成)会覆盖随机气泡,空闲时随机气泡保留。
					this.showBubble(this._pickBubble());
					if (this.options.onStatusToggle) this.options.onStatusToggle();
				};
			...
			/** 宠物名字(皮肤 · 情绪),显示在宠物下方;情绪变/换皮肤时刷新。 */
			_updateName() {
				if (!this._nameEl) return;
				const t = this.options.t;
				const skinName = this.options.skin === "blue-whale" ? t("petSkinBlue") : t("petSkinBlack");
				const emo = this._currentEmotion || "happy";
				const emoKey = "petEmotion" + emo.replace(/-(\w)/g, (_, c) => c.toUpperCase()).replace(/^\w/, (c) => c.toUpperCase());
				this._nameEl.textContent = skinName + " · " + t(emoKey);
			}
		}
		...
		class PetStatusPanel {
			...
			_build() {
				...
				el.className = "dsh-pet-status";
				el.innerHTML =
					'<div class="dsh-pet-status-head">' + t("petStatusTitle") +
					'<span class="dsh-pet-status-hint">' + t("petStatusHint") + "</span></div>" +
					...
					"</div><div class=\"dsh-pet-status-list\"></div>";
			...
			/** 锚定宠物上方居中;宠物贴近顶部则翻到下方(与气泡 below 逻辑一致)。 */
			position() {
				...
				if (rect.top - 12 - h >= 8) {
					...
					this.el.style.bottom = window.innerHeight - rect.bottom + 12 + "px";
				}
			}
			...
			/** 投影:失败(lastAgentError)> 运行中 > 已完成 > 待机。 */
			_project() {
				const snap = this.sessions.list.getSnapshot();
				const rows = [];
				for (const id of snap.ids || []) {
					...
					rows.push({ id, title: entry.displayTitle || id, status });
				}
				return rows;
			}
			...
			_render() {
				...
				if (rows.length === 0) {
					...
					this.listEl.innerHTML = rows.map((r) =>
						...
						(r.status === "running" ? t("petStatusRunning") : r.status === "failed" ? t("petStatusFailed") : r.status === "done" ? t("petStatusDone") : t("petStatusIdle")) +
						"</span></div>"
					).join("");
				}
				// 状态分布变化 → 气泡(3.5s)+ 情绪映射
			...
			_announce(fail, run, done) {
				const t = this.t;
				let msg = null;
				if (fail > 0) msg = t("petStatusBubbleFail", { n: fail, m: run });
				else if (run > 0) msg = t("petStatusBubbleRun", { m: run });
				else if (done > 0) msg = t("petStatusBubbleDone");
				if (msg) this.pet.showBubble(msg, 3500);
				// 情绪映射(规格:失败 > 运行中 > 完成 > 空闲):着急/忙碌/开心/打盹
		...
		let petStatusCtx = null; // 懒获取 sessions 服务(点击时才取,避免 apply 早于 runtime 时序问题)

		function petStatusToggle() {
			if (petStatusPanel) { petStatusClose(); return; }
			if (!petWidget || !petStatusCtx) return;
			const sessions = petStatusCtx.get("sessions");
			if (!sessions) return;
			petStatusPanel = new PetStatusPanel({ sessions, t: petBubbleT, pet: petWidget });
		}
		...
		function apply(ctx) {
			...
			// 状态概览数据源:平台已有投影(SessionRuntime 的 list + per-session notifier)。
			// 保存 ctx 引用,点击时才懒获取 sessions(避免插件 apply 早于 client-runtime 的时序问题)。
			petStatusCtx = ctx;
			const petCtl = {
