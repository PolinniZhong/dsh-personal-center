...
/** 按时段匹配条件规则,返回命中规则的文本(按数组顺序;空 text / 非法项跳过)。
 * v0.8.1:移除按模型条件(模型是用户主动选择的维度,不与插件耦合,见 PRD v0.8 §3),仅剩时段。 */
function matchRules(rules, nowMs) {
...
export function apply(ctx) {
	...
	ctx.on("system-prompt/assemble", async (assembly, context, next) => {
		...
		try {
			...
			const ruleTexts = matchRules(rules, Date.now());
					...
					if (url.pathname === "/personal-center/instructions") {
						...
						if (method === "POST") {
							...
							const templates = Array.isArray(body.templates)
								...
								: [];
							// v0.8 条件规则(按时段):过滤空 text;字段规范化(id/timeStart/timeEnd/text;不含 model)
