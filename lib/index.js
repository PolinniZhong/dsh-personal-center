...
export function apply(ctx) {
	...
	// v0.7:provider 收到 context(每次组装),按会话 cwd 最长前缀匹配工作区,
	// 有匹配返回「全局 + 工作区」合并文本,无匹配返回全局(与 v0.5.1 一致)。
	ctx.inject(["systemPrompt", "settings"], (promptCtx) => {
		...
		promptCtx.systemPrompt.variable("customInstructions", (context) => {
			const global = promptCtx.settings.get(NS)?.text ?? "";
			const cwd = context?.agent?.session?.header?.cwd ?? null;
			const { workspaces } = parseInstructions(promptCtx.settings);
			const wsText = cwd ? matchWorkspace(cwd, workspaces) : "";
			return mergeInstructions(global, wsText);
		});
	});
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (method === "GET" && url.pathname === "/personal-center/workspaces") {
						...
						let names = [];
						try {
							names = readdirSync(sessionsDir).map(decodeWorkspaceName).sort();
						} catch { /* 忽略 */ }
