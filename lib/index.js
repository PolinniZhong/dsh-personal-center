...
/** 宠物命名空间 schema(与浏览器端约定一致,posOverride 存 JSON 字符串)。 */
...
/** 宠物素材文件名白名单(防路径穿越)。 */
const PET_ASSET_NAMES = ["happy.webp", "busy.webp", "exhausted.webp", "money-pain.webp", "dozing.webp"];

/** 素材目录(lib/pet-assets/animations/,相对本文件)。 */
const PET_ASSET_DIR = fileURLToPath(new URL("./pet-assets/animations/", import.meta.url));
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (method === "GET" && url.pathname.startsWith("/personal-center/pet/assets/")) {
						const name = url.pathname.slice("/personal-center/pet/assets/".length);
						if (!PET_ASSET_NAMES.includes(name)) {
							sendJson(res, 404, { ok: false, error: "asset not found" });
							return;
						}
						if (!sendFile(res, 200, join(PET_ASSET_DIR, name), "image/webp", 86400)) {
