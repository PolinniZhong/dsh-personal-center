			...
			handler: async (req, res) => {
				...
				try {
					...
					if (method === "GET" && url.pathname.startsWith("/personal-center/pet/assets/")) {
						// 白名单:animations/<emo>.webp 或 idle/<emo>.webp,防路径穿越
						const rel = url.pathname.slice("/personal-center/pet/assets/".length);
						const m = /^(animations|idle)\/(happy|busy|exhausted|money-pain|dozing)\.webp$/.exec(rel);
						if (!m || PET_ASSET_KINDS.indexOf(m[1]) < 0 || PET_EMOTION_NAMES.indexOf(m[2]) < 0) {
