...
> 「个人配置」分区的视觉规范,全部取自 DSH 设计系统的实测值(从 `dsh-web-frontend` 与各设置分区组件提取)。改 UI 时遵守,保持与原生一致。
...
> **全局字号(v1.0 · 外观 tab)**:上表为「基准 14」下的取值;「外观」tab 的全局字号引擎 `UI_FONT_*` 允许用户整体偏移(11–16、默认 14),经 `<style>` 覆盖层按 delta 重声明,不改动上述令牌本身。
...
### 7.5 会话状态概览 UI 规范(v0.5)

> 宠物新增能力:实时显示会话状态(运行中 / 失败 / 已完成 / 待机)。实现详见 [DESKTOP-PET.md](DESKTOP-PET.md)。

**入口与交互**
- 入口:悬停宠物右上角「会话状态」按钮,或点击宠物本体(两者皆可);
- 打开面板时**隐藏数据气泡**,退出后恢复;
- 关闭:点面板外任意处 或 ESC(瞬态浮层范式,**无右上角关闭按钮**);
- 面板打开时宠物显示激活态(`interactive-bg-hover` 淡底)。

**毛玻璃面板(浮层)**
- 与气泡同款配方:半透明 `rgba(255,255,255,.45)`(深色 `rgba(28,30,36,.45)`)+ `backdrop-filter:blur(14px) saturate(1.5)` + 半透明描边 `rgba(255,255,255,.55)`(深色 `rgba(255,255,255,.14)`);
- 圆角 16px;宽 256px;**软阴影**(弱化层级,与气泡的"无阴影"区分——面板是内容容器可带极浅阴影);
- 头部:标题「会话概览」14px/500 + 弱化提示「点外部或 ESC 关闭」。

**状态色板(全用 `--dsw-alias-*`)**

| 状态 | 色令牌 | 用途 |
|---|---|---|
| 运行中 | `state-business-primary`(蓝) | 状态点 / 计数 / 徽标 |
| 失败 | `state-error-primary`(红) | 状态点 / 计数 / 徽标(**失败如实,绝不伪装成功**) |
| 已完成 | `state-success-primary`(绿) | 状态点 / 计数 / 徽标 |
| 待机 | `label-tertiary`(灰) | 状态点 / 徽标 |

- 徽标底:软底色用 `state-success-secondary`(成功)、`interactive-bg-hover`(待机);失败/运行中的软底无官方令牌,用本地软色(浅 `#fff1f0`/`#eef1ff`,深色对应加深,见代码);
- **摘要计数**:数字与标签同字号(**11px/600**,标签 11px `label-tertiary`),颜色同上;**数值为 0 的项不显示**(避免 0 干扰,只露有值的进行中/失败/完成);
- **列表行**:12px、行高固定 32px(`box-sizing:border-box`);**可见区恰好 5 整行**(`max-height:178px` + 滚动),运行中/失败置顶;悬停 `interactive-bg-hover`;行 `cursor:pointer` + 提示「双击进入该会话」;
- **过滤**:排除**归档**与**子代理子会话**(与左侧工作区一致:归档集合 + `origin !== "subagent"`)、空白会话不展示;
- **双击进入**:双击某条会话 → 平台 `sessions.open` 跳转到对应会话,面板自动关闭。

**数据与轻量(硬性)**
- 数据来源:平台已有 `sessions` 投影(useSessions),**事件驱动、零轮询**;
- 面板懒挂载、关闭即销毁;气泡 3.5s 自动消失;
- 零新依赖、纯本地;`enabled:false` 时不创建任何 DOM、不轮询。
