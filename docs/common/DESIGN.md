...
2. **素材格式:动画 WebP** 替代 GIF(GIF 二值透明有白边);由 `AI桌面宠物/black-whale-petblack-whale-pet-圆滚滚小黑鲸 V1/scripts/gif2webp.py` 从 RGBA 帧合成,产物提交进插件仓库;
3. **集成方式:宿主托管素材**(懒加载、bundle 保持轻量),而非 base64 内联;
4. **情绪判定顺序**:打盹先于开心(否则高缓存命中率用户永不入睡)。

### 5.4 v0.5 会话状态概览(已实现)

- **定位**:宠物新增「状态感知」——实时告诉用户哪些会话在跑、哪些失败(**失败如实标红,绝不伪装成功**),解决"界面撒谎"体验问题;
- **数据来源**:直接读平台已有 `sessions` 投影(客户端 `useSessions` 的 running/completed 等语义),**不自建状态机、不做 stuck 检测、不轮询**——事件驱动,零额外开销(本模块的硬性轻量约束,见 DESIGN-SYSTEM §7.5);
- **交互**:悬停宠物右上角「会话状态」按钮 / 点击宠物 → 毛玻璃面板;点面板外或 ESC 关闭;打开时隐藏数据气泡;
- **实现要点**:客户端纯前端;面板懒挂载、关闭即销毁;失败判定基于平台会话状态(completed 语义),不依赖后端探测;
- 详细规格与交接见 [宠物状态概览-设计规格与交接.md](宠物状态概览-设计规格与交接.md)。

### 5.5 v1.1 矢量桌宠皮肤(已实现,未发布)

- **定位**:在既有位图版(黑鲸/蓝鲸,动画 WebP)之外,新增**矢量极简版**(圆滚滚小黑/小蓝),走"丝滑鼠标跟随"产品线,非 WebP;
- **皮肤**:`black-vector` / `blue-vector`,纯 DOM(圆形渐变身体 + 胶囊眼),与位图皮肤并存、互斥;
- **交互**:鼠标跟随四层(眼睛朝向 / 头位移 / 上下"驼"形变 / 渐变光源)+ 转向透视(转向侧眼睛变小 + 两眼间距压缩)+ 靠近脸眨眼 / 点眼睛眯成线 / 点身体挠痒 / 随机头放大;
- **实现**:`VectorPetWidget` 内联进 `lib/client.js`(复用位图版拖拽 / 气泡 / 右键隐藏 / 状态按钮外壳);无情绪动画(情绪接口 no-op);位图↔矢量切换销毁重建实例;
- **宿主**:皮肤白名单扩为 4 个 + `PET_BITMAP_SKINS` 区分素材路由(矢量皮肤无素材)。
