...
| **宠物 · 浮层/配置** | pet 路由 + 素材托管 | `PetSection`(单页)+ `PetStatusConfig`(会话状态卡)+ `PetPanel`(宠物卡)+ `PetOverlay`(全局浮层) | `GET/POST /personal-center/pet`、`GET /personal-center/pet/assets/*` | [pet/DESKTOP-PET.md](pet/DESKTOP-PET.md) | 命名空间 `personal-center-pet`;素材 `lib/pet-assets/<skin>/` |
