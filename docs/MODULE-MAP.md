...
| **宠物 · 浮层/配置** | pet 路由 + 素材托管 | `PetPanel`(配置)+ `PetOverlay`(全局浮层)+ `PetOverlay` 状态概览 | `GET/POST /personal-center/pet`、`GET /personal-center/pet/assets/*` | [pet/DESKTOP-PET.md](pet/DESKTOP-PET.md) | 命名空间 `personal-center-pet`;素材 `lib/pet-assets/<skin>/` |
| **宠物 · 会话状态概览** | 数据来自平台 `sessions` 投影(客户端直取,零 host) | `PetStatusPanel`(毛玻璃面板,事件驱动) | —(无专用路由) | [pet/宠物状态概览-设计规格与交接.md](pet/宠物状态概览-设计规格与交接.md) | 平台 `ctx.get("sessions")` |
