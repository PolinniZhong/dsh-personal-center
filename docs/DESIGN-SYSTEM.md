...
- **宠物列表**:小黑鲸/小蓝鲸**各一张完整配置卡**(内容相同:预览图 56px 圆 + 标题(宠物名 + ⓘ)+ 今日统计 + 不透明度档位 + 情绪气泡 + 开关),`grid minmax(300px,1fr)` 并排;选中态 = 品牌蓝边框(`deepseek-400`)+ `interactive-bg-hover` 底;**点击卡片本体切换当前宠物**,点内部控件(开关/不透明度)不触发切换;
